...
<h2 id = '#sec:enforcing_dependency_version'> <a href = '#sec:enforcing_dependency_version'>覆盖传递依赖项版本</a> </h2>
...
<h3 id = '#sec:strict-version-consequences'> <a href = '#sec:strict-version-consequences'>使用严格版本的后果</a> </h3>
...
boolean-)强制依赖，并且不再建议使用：强制依赖存在一个排序问题，该问题很难诊断，并且无法与其他丰富版本约束一起很好地工作。您应该选择[严格的版本](#sec:enforcing_dependency_version)。如果你正在编写和发布一个[库](/md/了解库和应用程序之间的区别.md)，你还需要是知道`force`的
**不** 公开。  
...
<h2 id = '#sec:excluding-transitive-deps'> <a href = '#sec:excluding-transitive-deps'>排除传递依赖</a> </h2>
