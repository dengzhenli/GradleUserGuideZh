...
从历史上看，排除项还用作解决某些依赖项管理系统不支持的其他问题的辅助工具。但是，Gradle提供了可能更适合解决特定用例的各种功能。您可以考虑研究以下功能：
  ...
  * [解决相互排斥的依赖关系冲突](/md/处理互斥依赖性.md)：您经常看到用exclude解决的另一种情况是，两个依赖关系不能一起使用，因为它们表示同一事物（相同[功能](/md/处理互斥依赖性.md#组件功能介绍)）的两个实现。一个流行的示例是冲突日志记录API实现（如`log4j`和`log4j-over-slf4j`）或在不同版本（如`com.google.collections`和`guava`）中具有不同坐标的模块。在这种情况下，如果Gradle不知道此信息，则建议通过[声明组件功能中](/md/处理互斥依赖性.md#声明组件功能)所述的组件元数据规则添加缺少的功能信息。[](/md/处理互斥依赖性.md#声明组件功能)部分。即使您正在开发Library，而您的使用者也不得不再次解决冲突，将决定权交给Library的最终使用者通常是正确的解决方案。也就是说，作为Library作者，您最终不必决定消费者使用哪种日志记录实现。
