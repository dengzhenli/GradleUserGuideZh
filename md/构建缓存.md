...
内容
  ...
  * [总览](#sec:build_cache_intro)
  * [启用构建缓存](#sec:build_cache_enable)
  * [任务输出缓存](#sec:task_output_caching)
  * [可缓存的任务](#sec:task_output_caching_details)
  * [启用不可缓存任务的缓存](#enable_caching_of_non_cacheable_tasks)
  * [配置构建缓存](#sec:build_cache_configure)
  * [如何设置HTTP构建缓存后端](#sec:build_cache_setup_http_backend)
  * [实施自己的构建缓存](#sec:build_cache_implement)

╔═════════════════════════════  是否想学习顶尖工程团队用来保持构建快速和高性能的提示和技巧？[在此处注册](https://gradle.com/training/build-
cache-deep-dive/?bid=docs-build-cache)以获取我们的构建缓存培训。  
╚═════════════════════════════  
...
## [](#sec:build_cache_intro)[总览](#sec:build_cache_intro)

Gradle _构建缓存_
是一种缓存机制，旨在通过重用其他构建生成的输出来节省时间。构建缓存通过以下方式工作：存储（本地或远程）构建输出，并允许构建在确定输入未更改时从缓存中获取这些输出，从而避免了重新生成它们的昂贵工作。

使用构建缓存的第一个功能是 _任务输出缓存_
。本质上，任务输出缓存利用了与Gradle用于避免以前的本地版本已经产生一组任务输出[的最新检查](https://docs.gradle.org/6.7.1/userguide/more_about_tasks.html#sec:up_to_date_checks)相同的情报。但是，任务输出缓存不仅限于同一个工作空间中的先前版本，还使Gradle可以重用本地计算机上任何位置的任何早期版本的任务输出。当使用共享的构建缓存进行任务输出缓存时，这甚至可以在开发人员机器和构建代理之间使用。

除任务外，[工件转换](https://docs.gradle.org/6.7.1/userguide/artifact_transforms.html#sec:abm_artifact_transforms)还可以利用构建缓存并类似于任务输出缓存来重用其输出。
...
## [](#sec:build_cache_enable)[启用构建缓存](#sec:build_cache_enable)
...
启用构建缓存后，它将在Gradle用户主目录中存储构建输出。有关配置此目录或其他类型的构建缓存的信息，请参阅“[配置构建缓存”](#sec:build_cache_configure)。

## [](#sec:task_output_caching)[任务输出缓存](#sec:task_output_caching)

除了[最新检查中](https://docs.gradle.org/6.7.1/userguide/more_about_tasks.html#sec:up_to_date_checks)描述的增量构建外，Gradle还可以通过将任务的输入匹配来重用任务以前执行的输出，从而节省时间。任务输出可以通过构建缓存在一台计算机上的构建之间，甚至在不同计算机上运行的构建之间重复使用。
...
为了使您的构建能够很好地使用任务输出缓存，它必须与[增量构建](https://docs.gradle.org/6.7.1/userguide/more_about_tasks.html#sec:up_to_date_checks)功能一起正常工作。例如，当连续两次运行构建时，所有带有输出的任务都应为`UP-
TO-DATE`。在不满足此先决条件的情况下启用任务输出缓存时，您不能期望更快的构建或正确的构建。

启用构建缓存时，将自动启用任务输出缓存，请参阅[启用构建缓存](#sec:build_cache_enable)。

### [](#sec:task_output_caching_example)[它是什么样子的](#sec:task_output_caching_example)
...
`:classes`和`:assemble`是[生命周期的任务](https://docs.gradle.org/6.7.1/userguide/more_about_tasks.html#sec:lifecycle_tasks)，并`:processResources`
与`:jar`被复制类，因为它通常是更快地执行它们哪些是不可缓存的任务。

## [](#sec:task_output_caching_details)[可缓存的任务](#sec:task_output_caching_details)
...
下列输入[以与最新检查](https://docs.gradle.org/6.7.1/userguide/more_about_tasks.html#sec:how_does_it_work)相同的方式为任务的构建高速缓存键提供帮助：

  * 任务类型及其类路径

  * 输出属性的名称

  * 如[“自定义任务类型”一节中](https://docs.gradle.org/6.7.1/userguide/more_about_tasks.html#sec:task_input_output_annotations)所述，注释的属性名称和值[](https://docs.gradle.org/6.7.1/userguide/more_about_tasks.html#sec:task_input_output_annotations)
...
### [](#sec:task_output_caching_cacheable_tasks)[内置可缓存任务](#sec:task_output_caching_cacheable_tasks)
...
### [](#sec:task_output_caching_cacheable_3rd_party)[第三方插件](#sec:task_output_caching_cacheable_3rd_party)
...
### [](#sec:task_output_caching_inputs)[声明任务输入和输出](#sec:task_output_caching_inputs)

具有可缓存任务的输入和输出的完整图片非常重要，这样一个构建的结果可以安全地在其他地方重复使用。

缺少任务输入会导致不正确的缓存命中，其中不同的结果被视为相同，因为两个执行都使用相同的缓存键。如果Gradle不能完全捕获给定任务的所有输出，则缺少任务输出会导致构建失败。错误声明的任务输入可能导致高速缓存未命中，尤其是在包含易失性数据或绝对路径时。（有关应声明为输入和输出的信息，请参见[“任务输入和输出”一](https://docs.gradle.org/6.7.1/userguide/more_about_tasks.html#sec:task_inputs_outputs)节。）
...
为了处理任务的易失性输入，请考虑[配置输入标准化](https://docs.gradle.org/6.7.1/userguide/more_about_tasks.html#sec:configure_input_normalization)。

## [](#enable_caching_of_non_cacheable_tasks)[启用不可缓存任务的缓存](#enable_caching_of_non_cacheable_tasks)
...
### [](#using_annotations)[使用注释](#using_annotations)

我们创建的子类，`NpmTask`并使用[批注声明输入和输出](https://docs.gradle.org/6.7.1/userguide/more_about_tasks.html#sec:task_input_output_annotations)。
...
### [](#using_the_runtime_api)[使用运行时API](#using_the_runtime_api)

如果由于某种原因无法创建新的自定义任务类，则还可以使用[运行时API](https://docs.gradle.org/6.7.1/userguide/more_about_tasks.html#sec:task_input_output_runtime_api)声明输入和输出，从而使任务可缓存。
...
## [](#sec:build_cache_configure)[配置构建缓存](#sec:build_cache_configure)
...
### [](#sec:build_cache_configure_local)[内置本地构建缓存](#sec:build_cache_configure_local)
...
### [](#sec:build_cache_configure_remote)[远程HTTP构建缓存](#sec:build_cache_configure_remote)
...
### [](#sec:build_cache_configure_use_cases)[配置用例](#sec:build_cache_configure_use_cases)
...
也可以从[init脚本](https://docs.gradle.org/6.7.1/userguide/init_scripts.html#sec:using_an_init_script)配置构建缓存，该[脚本](https://docs.gradle.org/6.7.1/userguide/init_scripts.html#sec:using_an_init_script)可以在命令行中使用，可以添加到Gradle用户主目录中，也可以作为自定义Gradle发行版的一部分。
...
Gradle的[复合构建功能](https://docs.gradle.org/6.7.1/userguide/composite_builds.html#composite_builds)允许将其他完整的Gradle构建包括在内。无论所包含的构建是否自行定义构建缓存配置，此类包含的构建都会从顶级构建中继承构建缓存配置。

对于任何包含的构建，当前存在的构建缓存配置都会被有效忽略，而有利于顶层构建的配置。这也适用于`buildSrc`任何包含的内部版本的任何项目。

该[`buildSrc`目录](https://docs.gradle.org/6.7.1/userguide/organizing_gradle_projects.html#sec:build_sources)被视为[包含的build](https://docs.gradle.org/6.7.1/userguide/composite_builds.html#composite_build_intro)，因此它从顶级build继承了build缓存配置。

## [](#sec:build_cache_setup_http_backend)[如何设置HTTP构建缓存后端](#sec:build_cache_setup_http_backend)
...
## [](#sec:build_cache_implement)[实施自己的构建缓存](#sec:build_cache_implement)
