...
。本质上，任务输出缓存利用了与Gradle用于避免以前的本地版本已经产生一组任务输出[的最新检查](/md/处理任务_md#最新检查（又称增量构建）)相同的情报。但是，任务输出缓存不仅限于同一个工作空间中的先前版本，还使Gradle可以重用本地计算机上任何位置的任何早期版本的任务输出。当使用共享的构建缓存进行任务输出缓存时，这甚至可以在开发人员机器和构建代理之间使用。
...
除了[最新检查中](/md/处理任务_md#最新检查（又称增量构建）)描述的增量构建外，Gradle还可以通过将任务的输入匹配来重用任务以前执行的输出，从而节省时间。任务输出可以通过构建缓存在一台计算机上的构建之间，甚至在不同计算机上运行的构建之间重复使用。
...
为了使您的构建能够很好地使用任务输出缓存，它必须与[增量构建](/md/处理任务_md#最新检查（又称增量构建）)功能一起正常工作。例如，当连续两次运行构建时，所有带有输出的任务都应为`UP-
...
`:classes`和`:assemble`是[生命周期的任务](/md/处理任务_md#生命周期任务)，并`:processResources`
...
下列输入[以与最新检查](/md/处理任务_md#它是如何工作的？)相同的方式为任务的构建高速缓存键提供帮助：

  * 任务类型及其类路径

  * 输出属性的名称

  * 如[“自定义任务类型”一节中](/md/处理任务_md#自定义任务类型)所述，注释的属性名称和值[](/md/处理任务_md#自定义任务类型)
...
缺少任务输入会导致不正确的缓存命中，其中不同的结果被视为相同，因为两个执行都使用相同的缓存键。如果Gradle不能完全捕获给定任务的所有输出，则缺少任务输出会导致构建失败。错误声明的任务输入可能导致高速缓存未命中，尤其是在包含易失性数据或绝对路径时。（有关应声明为输入和输出的信息，请参见[“任务输入和输出”一](/md/处理任务_md#任务输入和输出)节。）
...
为了处理任务的易失性输入，请考虑[配置输入标准化](/md/处理任务_md#配置输入标准化)。
...
我们创建的子类，`NpmTask`并使用[批注声明输入和输出](/md/处理任务_md#自定义任务类型)。
...
如果由于某种原因无法创建新的自定义任务类，则还可以使用[运行时API](/md/处理任务_md#运行时API)声明输入和输出，从而使任务可缓存。
...
也可以从[init脚本](/md/初始化脚本_md#使用初始化脚本)配置构建缓存，该[脚本](/md/初始化脚本_md#使用初始化脚本)可以在命令行中使用，可以添加到Gradle用户主目录中，也可以作为自定义Gradle发行版的一部分。
...
该[`buildSrc`目录](/md/组织Gradle项目_md#用`buildSrc`抽象逻辑势在必行)被视为[包含的build](/md/复合构建_md#什么是复合构建？)，因此它从顶级build继承了build缓存配置。
