...
[的最新检查](/md/处理任务.md#最新检查（又称增量构建）)相同的情报。
...
除了[更新检查中](/md/处理任务.md#最新检查（又称增量构建）)描述的增量构建外，Gradle还可以通过将任务的输入匹配来重用任务以前执行的输出，从而节省时间。任务输出可以通过构建缓存在一台计算机上的构建之间，甚至在不同计算机上运行的构建之间重复使用。
