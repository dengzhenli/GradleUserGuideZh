...
<h2 id = '#sec:component_metadata_rules_details'> <a href = '#sec:component_metadata_rules_details'>元数据的哪些部分可以修改？</a> </h2>
...
可以调整每个变体的以下详细信息：
  ...
  * 变体的[依赖关系约束](/md/升级传递依赖的版本.md#sec:adding-constraints-transitive-deps)，包括[丰富版本](/md/声明丰富版本.md)-`withDependencyConstraints {}`块
...
整个组件的一些属性也可以更改：
  ...
  * [通过虚拟平台](/md/声明Library的能力.md#sec:declaring-capabilities-external-modules)进行[版本对齐](/md/声明Library的能力.md#sec:declaring-capabilities-external-modules)的 _belongsTo_ 属性[](/md/声明Library的能力.md#sec:declaring-capabilities-external-modules)
...
。这类似于在构建脚本中[定义可选功能变体](/md/建模功能变体和可选依赖项.md#sec::declare_feature_variants)。然后，我们使用一种方法来添加依赖项，以定义此可选功能所需的依赖项。
__[](/md/建模功能变体和可选依赖项.md#sec::declare_feature_variants)`add`

然后，在构建脚本中，我们可以将[依赖](/md/建模功能变体和可选依赖项.md#sec::consuming_feature_variants)项添加[到可选功能中](/md/建模功能变体和可选依赖项.md#sec::consuming_feature_variants)，Gradle将使用丰富的元数据来发现正确的传递性依赖项。
...
slf4j`）提供同一事物的替代实现。通过声明两者都提供相同的功能，Gradle在依赖图中仅接受其中之一。此示例以及如何用组件元数据规则解决该示例，在[功能建模](/md/声明Library的能力.md#sec:declaring-capabilities-external-modules)部分中进行了详细说明 。
...
Metadata的情况下对齐多个模块的版本。在[对齐未通过Gradle发布的模块版本](/md/对齐依赖版本.md#sec:align-versions-unpublished)的部分中对此进行了详细说明。

<h2 id = '#sec:custom_status_scheme'> <a href = '#sec:custom_status_scheme'>在组件级别上修改元数据以根据状态选择版本</a> </h2>
