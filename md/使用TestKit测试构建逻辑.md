...
API客户端](/md/Gradle和第三方工具.md#使用Tooling%20API嵌入Gradle)。它不包括[JUnit](http://junit.org/)，[TestNG](http://testng.org/)或任何其他测试执行框架的版本。必须明确声明这种依赖性。
...
API](/md/Gradle和第三方工具.md#使用Tooling%20API嵌入Gradle)执行构建。这意味着构建是在单独的过程中执行的（即，执行测试的过程不是同一过程）。因此，测试版本与测试过程不会共享相同的类路径或类加载器，并且测试代码不会隐式地提供给测试版本。
...
### [使用2.13之前的Gradle版本](#使用2.13之前的Gradle版本)
...
### [使用2.8之前的Gradle版本](#使用2.8之前的Gradle版本)
...
### [示例：将测试类下的代码注入2.8之前的Gradle版本的测试版本中](#示例：将测试类下的代码注入测试版本_for_gradle_versions_prior_to_2_8)
...
跑步者使用[Tooling API](/md/Gradle和第三方工具.md#使用Tooling%20API嵌入Gradle)执行构建。这意味着构建是在单独的过程中执行的（即，执行测试的过程不是同一过程）。因此，以调试方式执行
...
cache`参数传递给[GradleRunner](https://docs.gradle.org/6.7.1/javadoc/org/gradle/testkit/runner/GradleRunner.html)或使用[启用构建缓存中](/md/构建缓存.md#启用构建缓存)描述的其他方法之一。然后，可以在缓存插件的自定义任务时检查任务结果[TaskOutcome.FROM_CACHE](https://docs.gradle.org/6.7.1/javadoc/org/gradle/testkit/runner/TaskOutcome.html#FROM_CACHE)。此结果仅对Gradle
3.5及更高版本有效。
