...
内容
  ...
  * [用法](#用法)
  * [使用Gradle Runner进行功能测试](#使用Gradle_Runner进行功能测试)
...
API客户端](/md/Gradle和第三方工具.md#使用Tooling_API嵌入Gradle)。它不包括[JUnit](http://junit.org/)，[TestNG](http://testng.org/)或任何其他测试执行框架的版本。必须明确声明这种依赖性。
...
## [使用Gradle_Runner进行功能测试](#使用Gradle_Runner进行功能测试)
...
API](/md/Gradle和第三方工具.md#使用Tooling_API嵌入Gradle)执行构建。这意味着构建是在单独的过程中执行的（即，执行测试的过程不是同一过程）。因此，测试版本与测试过程不会共享相同的类路径或类加载器，并且测试代码不会隐式地提供给测试版本。
