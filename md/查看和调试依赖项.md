...
Gradle可以可视化项目中每个可用[配置](https://docs.gradle.org/6.7.1/userguide/declaring_dependencies.html#sec:what-are-dependency-configurations)的整个依赖树。
...
项目上执行。如果您在根项目上运行任务，它将显示根项目而不是任何子项目的依赖项。确保在运行时始终以[正确的项目](https://docs.gradle.org/6.7.1/userguide/command_line_interface.html#executing_tasks_in_multi_project_builds)为目标`dependencies`。  
  
╚═════════════════════════════    
  
假设您要为使用[JGit库](https://www.eclipse.org/jgit/)执行SCM操作（例如，为发布过程建模）的项目创建任务。您可以在[自定义配置](https://docs.gradle.org/6.7.1/userguide/declaring_dependencies.html#sec:what-are-dependency-configurations)的帮助下声明任何外部工具的依赖关系，这样它就不会污染其他上下文，例如生产源代码的编译类路径。
...
大型软件项目不可避免地会通过直接或传递依赖关系来处理越来越多的依赖关系。该[依赖性报告](#sec:listing_dependencies)为您提供具有相关性的原始名单，但并没有解释
_为什么_ 它们已经被选择或者 _其_ 依赖性负责拉他们到图形。

让我们看一个具体的例子。一个项目可以请求相同依赖项的两个不同版本，无论是直接依赖项还是传递依赖项。Gradle应用[版本冲突解决方案](https://docs.gradle.org/6.7.1/userguide/dependency_resolution.html#sec:version-conflict)来确保依赖关系图中仅存在一个版本的依赖关系。在此示例中，冲突的依赖关系由表示`commons-codec:commons-codec`。
...
项目上执行。如果您在根项目上运行任务，它将显示根项目而不是任何子项目的依赖项信息。确保在运行时始终以[正确的项目](https://docs.gradle.org/6.7.1/userguide/command_line_interface.html#executing_tasks_in_multi_project_builds)为目标`dependencyInsight`。  
...
有关配置的更多信息，请参阅有关声明依赖项的文档，该文档描述了[什么是依赖项配置](https://docs.gradle.org/6.7.1/userguide/declaring_dependencies.html#sec:what-are-dependency-configurations)。
...
要求：<文本>|依存关系出现在图形中，包含项带有[`because`文本](https://docs.gradle.org/6.7.1/userguide/declaring_dependencies.html#sec:documenting-dependencies)。  
要求：与版本<versions>不匹配|该依赖关系将显示在图形中，并带有[动态版本](https://docs.gradle.org/6.7.1/userguide/dynamic_versions.html#sub:declaring_dependency_with_dynamic_version)，其中不包括列出的版本。这之后也可以是`because`文本。  
要求：拒绝版本<versions>|依赖关系显示在图中，其中包含一个或多个[丰富版本](https://docs.gradle.org/6.7.1/userguide/rich_versions.html)`reject`。这之后也可以是`because`文本。  
通过冲突解决：版本之间<version>|依赖关系在图中出现了多次，具有不同的版本请求。这样可以[解决冲突，](https://docs.gradle.org/6.7.1/userguide/dependency_resolution.html#sec:version-conflict)从而选择最合适的版本。  
受约束|一个[依赖约束](https://docs.gradle.org/6.7.1/userguide/dependency_constraints.html#sec:adding-constraints-transitive-deps)参加了版本选择。这之后也可以是`because`文本。  
由祖先|图中有一个带有的[丰富版本](https://docs.gradle.org/6.7.1/userguide/rich_versions.html)，`strictly`用于强制执行此依赖项的版本。  
按规则选择|一个[依赖解决规则](https://docs.gradle.org/6.7.1/userguide/resolution_rules.html)推翻默认选择过程。这之后也可以是`because`文本。  
...
拒绝：版本<版本>：<属性信息>|依赖项具有动态版本，某些版本与[请求的属性](https://docs.gradle.org/6.7.1/userguide/variant_model.html#sec:variant-aware-matching)不匹配。  
被迫|该构建会[强制执行](https://docs.gradle.org/6.7.1/userguide/dependency_downgrade_and_exclude.html#forced_dependencies_vs_strict_dependencies)依赖项的版本。  
 
请注意，如果图形中存在多个选择原因，则将全部列出。

## [](#sec:resolving-version-conflict)[解决版本冲突](#sec:resolving-version-conflict)

如果所选版本与您的期望不符，则Gradle提供了一系列工具来帮助您[控制传递依赖](https://docs.gradle.org/6.7.1/userguide/dependency_constraints.html)。

## [](#sec:resolving-variant-aware-errors)[解决变量选择错误](#sec:resolving-variant-aware-errors)

有时会在[变体选择级别上](https://docs.gradle.org/6.7.1/userguide/variant_model.html)发生选择错误。请查看[专用部分](https://docs.gradle.org/6.7.1/userguide/variant_model.html#sec:variant-select-errors)以了解这些错误以及如何解决它们。
...
在大多数情况下，可以通过在另一个项目上创建跨项目的依赖关系来解决此问题。有关更多信息，请参见用于[在项目之间共享输出](https://docs.gradle.org/6.7.1/userguide/cross_project_publications.html#cross_project_publications)的文档。如果您发现使用这些技术无法解决的用例，请通过提交遵循我们的发行准则的[GitHub发行](https://github.com/gradle/gradle/issues)让我们知道。
