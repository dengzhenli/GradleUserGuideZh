...
<h2 id = '#sec:debugging-build-scans'> <a href = '#sec:debugging-build-scans'>构建扫描</a> </h2>
...
<h2 id = '#sec:listing_dependencies'> <a href = '#sec:listing_dependencies'>列出项目中的依赖项</a> </h2>

Gradle可以可视化项目中每个可用[配置](/md/声明依赖.md#sec:what-are-dependency-configurations)的整个依赖树。
...
假设您要为使用[JGit库](https://www.eclipse.org/jgit/)执行SCM操作（例如，为发布过程建模）的项目创建任务。您可以在[自定义配置](/md/声明依赖.md#sec:what-are-dependency-configurations)的帮助下声明任何外部工具的依赖关系，这样它就不会污染其他上下文，例如生产源代码的编译类路径。
...
<h2 id = '#sec:identifying_reason_dependency_selection'> <a href = '#sec:identifying_reason_dependency_selection'>识别选择了哪个依赖版本以及为什么</a> </h2>
...
让我们看一个具体的例子。一个项目可以请求相同依赖项的两个不同版本，无论是直接依赖项还是传递依赖项。Gradle应用[版本冲突解决方案](/md/了解依赖性解析.md#sec:version-conflict)来确保依赖关系图中仅存在一个版本的依赖关系。在此示例中，冲突的依赖关系由表示`commons-codec:commons-codec`。
...
有关配置的更多信息，请参阅有关声明依赖项的文档，该文档描述了[什么是依赖项配置](/md/声明依赖.md#sec:what-are-dependency-configurations)。
...
要求：<文本>|依存关系出现在图形中，包含项带有[`because`文本](/md/声明依赖.md#sec:documenting-dependencies)。  
要求：与版本<versions>不匹配|该依赖关系将显示在图形中，并带有[动态版本](/md/处理随时间变化的版本.md#sub:declaring_dependency_with_dynamic_version)，其中不包括列出的版本。这之后也可以是`because`文本。  
要求：拒绝版本<versions>|依赖关系显示在图中，其中包含一个或多个[丰富版本](/md/声明丰富版本.md)`reject`。这之后也可以是`because`文本。  
通过冲突解决：版本之间<version>|依赖关系在图中出现了多次，具有不同的版本请求。这样可以[解决冲突，](/md/了解依赖性解析.md#sec:version-conflict)从而选择最合适的版本。  
受约束|一个[依赖约束](/md/升级传递依赖的版本.md#sec:adding-constraints-transitive-deps)参加了版本选择。这之后也可以是`because`文本。  
...
<h2 id = '#sec:resolving-version-conflict'> <a href = '#sec:resolving-version-conflict'>解决版本冲突</a> </h2>

如果所选版本与您的期望不符，则Gradle提供了一系列工具来帮助您[控制传递依赖](/md/升级传递依赖的版本.md)。

<h2 id = '#sec:resolving-variant-aware-errors'> <a href = '#sec:resolving-variant-aware-errors'>解决变量选择错误</a> </h2>

有时会在[变体选择级别上](/md/使用变体属性.md)发生选择错误。请查看[专用部分](/md/使用变体属性.md#sec:variant-select-errors)以了解这些错误以及如何解决它们。

<h2 id = '#sub:resolving-unsafe-configuration-resolution-errors'> <a href = '#sub:resolving-unsafe-configuration-resolution-errors'>解决不安全的配置解析错误</a> </h2>
