...
内容
  ...
  * [AWS S3存储库配置](#AWS%20S3存储库配置)
  * [Google Cloud Storage储存库配置](#Google%20Cloud%20Storage储存库配置)
  * [处理凭证](#处理凭证)
...
构建软件的组织可能希望利用公共二进制存储库来下载和使用开源依赖项。流行的公共存储库包括[Maven Central](#Maven%20Central存储库)，
[Bintray JCenter](#JCenter%20Maven存储库)和[Google Android](#Google%20Maven存储库)存储库。
...
### [Maven%20Central存储库](#Maven%20Central存储库)
...
### [JCenter%20Maven存储库](#JCenter%20Maven存储库)
...
### [Google%20Maven存储库](#Google%20Maven存储库)
...
API上可用的相应方法，可以将具有自定义URL的存储库指定为Maven或Ivy存储[库](https://docs.gradle.org/6.7.1/dsl/org.gradle.api.artifacts.dsl.RepositoryHandler.html)。Gradle支持其他协议，而不是自定义URL`http`或`https`作为自定义URL的一部分，例如`file`，`sftp`或`s3`。有关完整信息，请参见[有关受支持的存储库类型](#支持的存储库类型)的[部分](#支持的存储库类型)。

您还[可以](#定义Ivy存储库的自定义模式布局)通过使用`ivy
{
}`存储库来[定义自己的存储库布局](#定义Ivy存储库的自定义模式布局)，因为它们在存储模块中模块的组织方式上非常灵活。
...
下载该模块的所有工件。您可以了解有关[依赖项下载](/md/了解依赖性解析_md#Gradle如何检索依​​赖元数据？)的内部工作的更多信息。  
...
Gradle在格式和连接性方面都支持广泛的依赖源。您可以通过以下方式解决依赖关系：
  ...
  * 不同格式
    ...
    * 一个与[Maven兼容的](#Maven仓库)工件存储库（例如：Maven Central，JCenter等）

    * 一个[常青藤兼容](#Ivy仓库)神器库（包括自定义布局）
...
配置了这样的存储库后，Gradle将完全绕过它的[依赖项缓存](/md/了解依赖性解析_md#依赖缓存)，因为无法保证内容在两次执行之间不会更改。由于该限制，它们可能会对性能产生影响。
...
Gradle将已解决的依赖性存储在[其自己的缓存中](/md/了解依赖性解析_md#依赖缓存)。即使您从基于Maven的远程存储库中解决依赖关系，构建也无需声明本地Maven存储库。  
...
作为一般建议，应避免添加`mavenLocal()`为存储库。使用`mavenLocal()`时应注意不同的问题：
  ...
  * Maven将其用作缓存而不是存储库，这意味着它可以包含部分模块。
    ...
    * 例如，如果Maven从不下载给定模块的源文件或javadoc文件，则Gradle将不会找到它们，因为一旦找到模块，它就会[在单个存储库中搜索文件](/md/了解依赖性解析_md#Gradle如何检索依​​赖元数据？)。
  ...
  * 为了减轻该元数据和/或伪影可以改变的，Gradle不执行的事实[任何缓存](/md/了解依赖性解析_md#依赖缓存)对[本地资源库](#本地存储库)
...
使用[存储库级内容过滤](#声明存储库过滤器)器声明的[过滤器](#声明存储库过滤器)不是唯一的。这意味着声明一个存储库 _包含_
...
另外，Gradle提供了一个API，可让您声明存储库 _专门包含_ 工件。如果这样做：
  ...
  * 专有存储库内容必须在扩展中声明（就像[存储库级内容一样](#声明存储库过滤器)）
...
第一阶段是[配置构建](/md/构建生命周期_md#构建阶段)并加载其应用的插件时。为此，Gradle将使用一组特殊的存储库。
...
Repositories](/md/使用Gradle插件_md#自定义插件存储库)。
...
`s3`|访问密钥/秘密密钥/会话令牌或环境变量|[文献资料](#AWS%20S3存储库配置)  
`gcs`|[默认应用程序凭据](https://developers.google.com/identity/protocols/application-default-credentials)来自知名文件，环境变量等。|[文献资料](#Google%20Cloud%20Storage储存库配置)  
...
有关与AWS S3相关的身份验证的详细信息，请参阅“ [AWS S3存储库配置](#AWS%20S3存储库配置)”部分。
...
### [AWS%20S3%20V4签名（AWS4-HMAC-SHA256）](#AWS%20S3%20V4签名（AWS4-HMAC-SHA256）)
...
## [Google%20Cloud%20Storage储存库配置](#Google%20Cloud%20Storage储存库配置)
...
在需要它们的构建期间，
将从[Gradle属性](/md/Gradle环境搭建_md#Gradle属性)中查找凭据值。
