...
可以在其中指定许多命令行标志，`gradle.properties`以避免需要键入。有关详细信息，请参见《[配置构建环境指南](/md/Gradle环境搭建.md#sec:gradle_configuration_properties)》。  
  
╚═════════════════════════════    
  
以下各节介绍了Gradle命令行界面的使用，大致按用户目标分组。一些插件还添加了自己的命令行选项，例如[`--tests`Java测试过滤](/md/在Java和JVM项目中进行测试.md#test_filtering)。有关为您自己的任务公开命令行选项的更多信息，请参阅《[声明和使用命令行选项》](/md/开发自定义Gradle任务类型.md#sec:declaring_and_using_command_line_options)。

<h2 id = '#sec:command_line_executing_tasks'> <a href = '#sec:command_line_executing_tasks'>执行任务</a> </h2>

您可以运行任务及其所有[依赖项](/md/构建脚本基础.md#sec:task_dependencies)。
...
大多数构建都支持一组常见的任务，称为[
_生命周期任务_](/md/处理任务.md#sec:lifecycle_tasks)
...
<h3 id = '#sec:excluding_tasks_from_the_command_line'> <a href = '#sec:excluding_tasks_from_the_command_line'>从执行中排除任务</a> </h3>
...
<h3 id = '#sec:rerun_tasks'> <a href = '#sec:rerun_tasks'>强制执行任务</a> </h3>

您可以使用以下选项强制Gradle执行所有任务，而忽略[最新检查](/md/处理任务.md#sec:up_to_date_checks)`--rerun-
tasks`：
...
<h3 id = '#sec:continue_build_on_failure'> <a href = '#sec:continue_build_on_failure'>发生故障时继续构建</a> </h3>
...
<h3 id = '#sec:name_abbreviation'> <a href = '#sec:name_abbreviation'>名称缩写</a> </h3>
...
<h2 id = '#sec:command_line_project_reporting'> <a href = '#sec:command_line_project_reporting'>项目报告</a> </h2>
...
<h3 id = '#sec:listing_tasks'> <a href = '#sec:listing_tasks'>上市任务</a> </h3>
...
<h3 id = '#sec:show_task_details'> <a href = '#sec:show_task_details'>显示任务使用情况详细信息</a> </h3>
...
<h3 id = '#sec:listing_properties'> <a href = '#sec:listing_properties'>列出项目属性</a> </h3>
...
<h2 id = '#sec:command_line_completion'> <a href = '#sec:command_line_completion'>命令行完成</a> </h2>
...
<h2 id = '#sec:command_line_debugging'> <a href = '#sec:command_line_debugging'>调试选项</a> </h2>
...
<h2 id = '#sec:command_line_performance'> <a href = '#sec:command_line_performance'>性能选项</a> </h2>

优化构建性能时，请尝试以下选项。[在此处](https://guides.gradle.org/performance/)了解有关[提高Gradle构建的性能的](https://guides.gradle.org/performance/)更多信息。

可以在其中指定许多这些选项，`gradle.properties`因此不需要命令行标志。请参阅《[配置构建环境指南》](/md/Gradle环境搭建.md#sec:gradle_configuration_properties)。
...
切换[按需配置](/md/配置时间和执行时间.md#sec:configuration_on_demand)。在此构建运行中仅配置相关项目。
_默认为关闭_ 。
...
并行构建项目。有关此选项的限制，请参见[并行项目执行](/md/配置时间和执行时间.md#sec:parallel_execution)。
_默认为关闭_ 。
...
<h2 id = '#sec:command_line_logging'> <a href = '#sec:command_line_logging'>记录选项</a> </h2>
...
<h3 id = '#sec:command_line_customizing_log_format'> <a href = '#sec:command_line_customizing_log_format'>自定义日志格式</a> </h3>
...
<h3 id = '#sec:command_line_warnings'> <a href = '#sec:command_line_warnings'>显示或隐藏警告</a> </h3>
...
通过[Gradle属性](/md/Gradle环境搭建.md#sec:gradle_configuration_properties)指定警告模式。下文将介绍不同的模式。
...
<h3 id = '#sec:rich_console'> <a href = '#sec:rich_console'>丰富控制台</a> </h3>
...
<h2 id = '#sec:command_line_execution_options'> <a href = '#sec:command_line_execution_options'>执行选项</a> </h2>
...
指定构建应在不访问网络资源的情况下运行。了解有关[覆盖依赖项缓存的选项的](/md/处理随时间变化的版本.md#sec:controlling_dependency_caching_command_line)更多信息。

`--refresh-dependencies`

    

刷新依赖项状态。在[依赖管理文档中](/md/处理随时间变化的版本.md#sec:controlling_dependency_caching_command_line)了解有关如何使用它的更多信息。
...
不要重建项目依赖项。对于[调试和微调`buildSrc`](/md/组织Gradle项目.md#sec:build_sources)很有用，但可能导致错误的结果。请谨慎使用！
...
设置JVM的系统属性，例如`-Dmyprop=myvalue`。请参阅[系统属性](/md/Gradle环境搭建.md#sec:gradle_system_properties)。
...
设置根项目的项目属性，例如`-Pmyprop=myvalue`。请参阅[项目属性](/md/Gradle环境搭建.md#sec:project_properties)。
...
<h2 id = '#sec:command_line_bootstrapping_projects'> <a href = '#sec:command_line_bootstrapping_projects'>引导新项目</a> </h2>
...
<h2 id = '#sec:continuous_build'> <a href = '#sec:continuous_build'>持续构建</a> </h2>
...
<h4 id = '#sec:build_cycles'> <a href = '#sec:build_cycles'>建造周期</a> </h4>
...
<h4 id = '#sec:continuous_build_limitations_jdk9'> <a href = '#sec:continuous_build_limitations_jdk9'>Java 9的限制</a> </h4>
...
<h4 id = '#sec:performance_and_stability'> <a href = '#sec:performance_and_stability'>性能稳定</a> </h4>
...
<h4 id = '#sec:changes_to_symbolic_links'> <a href = '#sec:changes_to_symbolic_links'>更改符号链接</a> </h4>
...
<h4 id = '#sec:changes_to_build_logic_are_not_considered'> <a href = '#sec:changes_to_build_logic_are_not_considered'>不考虑更改构建逻辑</a> </h4>
