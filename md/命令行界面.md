...
内容
  ...
  * [执行任务](#sec_command_line_executing_tasks)
  * [常见任务](#common_tasks)
  * [项目报告](#sec_command_line_project_reporting)
  * [命令行完成](#sec_command_line_completion)
  * [调试选项](#sec_command_line_debugging)
  * [性能选项](#sec_command_line_performance)
  * [记录选项](#sec_command_line_logging)
  * [执行选项](#sec_command_line_execution_options)
  * [环境选项](#environment_options)
  * [引导新项目](#sec_command_line_bootstrapping_projects)
  * [持续构建](#sec_continuous_build)

命令行界面是与Gradle交互的主要方法之一。以下内容作为执行和自定义Gradle使用命令行或编写脚本或配置持续集成时的参考。

强烈建议使用[Gradle包装器](/md/gradle_wrapper.md#gradle_wrapper)。您应该取代`./gradlew`或`gradlew.bat`为`gradle`使用包装时，在所有下面的例子。
...
可以在其中指定许多命令行标志，`gradle.properties`以避免需要键入。有关详细信息，请参见《[配置构建环境指南](/md/Gradle环境搭建.md#sec_gradle_configuration_properties)》。  
  
╚═════════════════════════════    
  
以下各节介绍了Gradle命令行界面的使用，大致按用户目标分组。一些插件还添加了自己的命令行选项，例如[`--tests`Java测试过滤](/md/在Java和JVM项目中进行测试.md#test_filtering)。有关为您自己的任务公开命令行选项的更多信息，请参阅《[声明和使用命令行选项》](/md/开发自定义Gradle任务类型.md#sec_declaring_and_using_command_line_options)。

<h2 id = '#sec_command_line_executing_tasks'> <a href = '#sec_command_line_executing_tasks'>执行任务</a> </h2>

您可以运行任务及其所有[依赖项](/md/构建脚本基础.md#sec_task_dependencies)。

    
    
    $ gradle myTask

您可以在“[项目报告”部分中](#sec_command_line_project_reporting)了解哪些项目和任务可用。

大多数构建都支持一组常见的任务，称为[
_生命周期任务_](/md/处理任务.md#sec_lifecycle_tasks)
。这些措施包括`build`，`assemble`和`check`任务。

<h3 id = '#executing_tasks_in_multi_project_builds'> <a href = '#executing_tasks_in_multi_project_builds'>在多项目构建中执行任务</a> </h3>
...
<h3 id = '#executing_multiple_tasks'> <a href = '#executing_multiple_tasks'>执行多项任务</a> </h3>
...
<h3 id = '#sec_excluding_tasks_from_the_command_line'> <a href = '#sec_excluding_tasks_from_the_command_line'>从执行中排除任务</a> </h3>
...
<h3 id = '#sec_rerun_tasks'> <a href = '#sec_rerun_tasks'>强制执行任务</a> </h3>

您可以使用以下选项强制Gradle执行所有任务，而忽略[最新检查](/md/处理任务.md#sec_up_to_date_checks)`--rerun-
tasks`：
...
<h3 id = '#sec_continue_build_on_failure'> <a href = '#sec_continue_build_on_failure'>发生故障时继续构建</a> </h3>
...
<h3 id = '#sec_name_abbreviation'> <a href = '#sec_name_abbreviation'>名称缩写</a> </h3>
...
<h2 id = '#common_tasks'> <a href = '#common_tasks'>常见任务</a> </h2>

以下是内置和大多数主要Gradle插件应用的任务约定。

<h3 id = '#computing_all_outputs'> <a href = '#computing_all_outputs'>计算所有输出</a> </h3>
...
<h3 id = '#running_applications'> <a href = '#running_applications'>运行应用程序</a> </h3>
...
<h3 id = '#running_all_checks'> <a href = '#running_all_checks'>运行所有检查</a> </h3>
...
<h3 id = '#cleaning_outputs'> <a href = '#cleaning_outputs'>清洁输出</a> </h3>
...
<h2 id = '#sec_command_line_project_reporting'> <a href = '#sec_command_line_project_reporting'>项目报告</a> </h2>

Gradle提供了一些内置任务，这些任务显示了构建的特定细节。这对于理解构建的结构和依赖性以及调试问题很有用。

您可以使用来获得有关可用报告选项的基本帮助`gradle help`。

<h3 id = '#listing_projects'> <a href = '#listing_projects'>上市项目</a> </h3>
...
<h3 id = '#sec_listing_tasks'> <a href = '#sec_listing_tasks'>上市任务</a> </h3>
...
<h3 id = '#sec_show_task_details'> <a href = '#sec_show_task_details'>显示任务使用情况详细信息</a> </h3>
...
<h3 id = '#reporting_dependencies'> <a href = '#reporting_dependencies'>报告依赖性</a> </h3>
...
<h3 id = '#listing_project_dependencies'> <a href = '#listing_project_dependencies'>列出项目依赖项</a> </h3>
...
<h3 id = '#sec_listing_properties'> <a href = '#sec_listing_properties'>列出项目属性</a> </h3>
...
<h3 id = '#software_model_reports'> <a href = '#software_model_reports'>软件模型报告</a> </h3>
...
<h2 id = '#sec_command_line_completion'> <a href = '#sec_command_line_completion'>命令行完成</a> </h2>
...
<h2 id = '#sec_command_line_debugging'> <a href = '#sec_command_line_debugging'>调试选项</a> </h2>
...
打印出完整的（非常详细的）堆栈跟踪信息，以了解任何异常。另请参阅[日志记录选项](#sec_command_line_logging)。

`-s`， `--stacktrace`

    

还打印出堆栈跟踪信息以了解用户异常（例如，编译错误）。另请参阅[日志记录选项](#sec_command_line_logging)。
...
<h2 id = '#sec_command_line_performance'> <a href = '#sec_command_line_performance'>性能选项</a> </h2>

优化构建性能时，请尝试以下选项。[在此处](https://guides.gradle.org/performance/)了解有关[提高Gradle构建的性能的](https://guides.gradle.org/performance/)更多信息。

可以在其中指定许多这些选项，`gradle.properties`因此不需要命令行标志。请参阅《[配置构建环境指南》](/md/Gradle环境搭建.md#sec_gradle_configuration_properties)。
...
切换[按需配置](/md/配置时间和执行时间.md#sec_configuration_on_demand)。在此构建运行中仅配置相关项目。
_默认为关闭_ 。
...
并行构建项目。有关此选项的限制，请参见[并行项目执行](/md/配置时间和执行时间.md#sec_parallel_execution)。
_默认为关闭_ 。
...
<h3 id = '#gradle_daemon_options'> <a href = '#gradle_daemon_options'>Gradle守护程序选项</a> </h3>
...
<h2 id = '#sec_command_line_logging'> <a href = '#sec_command_line_logging'>记录选项</a> </h2>

<h3 id = '#setting_log_level'> <a href = '#setting_log_level'>设置日志级别</a> </h3>
...
<h3 id = '#sec_command_line_customizing_log_format'> <a href = '#sec_command_line_customizing_log_format'>自定义日志格式</a> </h3>
...
<h3 id = '#sec_command_line_warnings'> <a href = '#sec_command_line_warnings'>显示或隐藏警告</a> </h3>
...
通过[Gradle属性](/md/Gradle环境搭建.md#sec_gradle_configuration_properties)指定警告模式。下文将介绍不同的模式。
...
<h3 id = '#sec_rich_console'> <a href = '#sec_rich_console'>丰富控制台</a> </h3>
...
<h2 id = '#sec_command_line_execution_options'> <a href = '#sec_command_line_execution_options'>执行选项</a> </h2>
...
指定构建应在不访问网络资源的情况下运行。了解有关[覆盖依赖项缓存的选项的](/md/处理随时间变化的版本.md#sec_controlling_dependency_caching_command_line)更多信息。

`--refresh-dependencies`

    

刷新依赖项状态。在[依赖管理文档中](/md/处理随时间变化的版本.md#sec_controlling_dependency_caching_command_line)了解有关如何使用它的更多信息。
...
不要重建项目依赖项。对于[调试和微调`buildSrc`](/md/组织Gradle项目.md#sec_build_sources)很有用，但可能导致错误的结果。请谨慎使用！

<h2 id = '#environment_options'> <a href = '#environment_options'>环境选项</a> </h2>
...
设置JVM的系统属性，例如`-Dmyprop=myvalue`。请参阅[系统属性](/md/Gradle环境搭建.md#sec_gradle_system_properties)。
...
设置根项目的项目属性，例如`-Pmyprop=myvalue`。请参阅[项目属性](/md/Gradle环境搭建.md#sec_project_properties)。
...
<h2 id = '#sec_command_line_bootstrapping_projects'> <a href = '#sec_command_line_bootstrapping_projects'>引导新项目</a> </h2>

<h3 id = '#creating_new_gradle_builds'> <a href = '#creating_new_gradle_builds'>创建新的Gradle版本</a> </h3>
...
<h3 id = '#standardize_and_provision_gradle'> <a href = '#standardize_and_provision_gradle'>标准化和配置Gradle</a> </h3>
...
version`。有关如何使用这些选项的完整详细信息，请参见[Gradle包装器部分](/md/gradle_wrapper.md#gradle_wrapper)。

<h2 id = '#sec_continuous_build'> <a href = '#sec_continuous_build'>持续构建</a> </h2>
...
<h3 id = '#terminating_continuous_build'> <a href = '#terminating_continuous_build'>终止连续构建</a> </h3>
...
<h3 id = '#continuous_build_limitations'> <a href = '#continuous_build_limitations'>局限性和怪癖</a> </h3>

当前连续构建的实现要注意几个问题。这些可能会在将来的Gradle版本中解决。

<h4 id = '#sec_build_cycles'> <a href = '#sec_build_cycles'>建造周期</a> </h4>
...
<h4 id = '#sec_continuous_build_limitations_jdk9'> <a href = '#sec_continuous_build_limitations_jdk9'>Java 9的限制</a> </h4>
...
<h4 id = '#sec_performance_and_stability'> <a href = '#sec_performance_and_stability'>性能稳定</a> </h4>
...
<h4 id = '#sec_changes_to_symbolic_links'> <a href = '#sec_changes_to_symbolic_links'>更改符号链接</a> </h4>
...
<h4 id = '#sec_changes_to_build_logic_are_not_considered'> <a href = '#sec_changes_to_build_logic_are_not_considered'>不考虑更改构建逻辑</a> </h4>
