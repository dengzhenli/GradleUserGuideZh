...
本节介绍了Gradle提供的直接影响依赖关系解析引擎行为的机制。与本章介绍的其他概念（例如[依赖项约束](https://docs.gradle.org/6.7.1/userguide/dependency_constraints.html)或[组件元数据规则）不同](https://docs.gradle.org/6.7.1/userguide/component_metadata_rules.html)，它们都是决议的
**输入** ，以下机制使您可以编写直接注入决议引擎的规则。因此，它们可以看作是 _蛮力_
解决方案，可能会隐藏未来的问题（例如，如果添加了新的依赖项）。因此，一般建议仅在其他手段不足的情况下使用以下机制。如果要编写[库](https://docs.gradle.org/6.7.1/userguide/library_vs_application.html)，则应始终首选[依赖项约束](https://docs.gradle.org/6.7.1/userguide/dependency_constraints.html)
因为它们是为您的消费者发布的。  
...
使用具有[丰富版本约束](https://docs.gradle.org/6.7.1/userguide/rich_versions.html#rich-version-constraints)的 _reject_
...
（您不应使用此版本）。[](https://docs.gradle.org/6.7.1/userguide/rich_versions.html#rich-version-constraints) __ __ __  
  
╚═════════════════════════════    
  
## [](#sec:module_replacement)[使用模块更换规则](#sec:module_replacement)

就[功能冲突](https://docs.gradle.org/6.7.1/userguide/dependency_capability_conflict.html#sub:capabilities)而言，最好表达模块[冲突](https://docs.gradle.org/6.7.1/userguide/dependency_capability_conflict.html#sub:capabilities)。但是，如果没有声明这样的规则，或者您正在使用不支持功能的Gradle版本，则Gradle提供了解决这些问题的工具。
...
Gradle的依赖性管理引擎具有[变体感知功能，](https://docs.gradle.org/6.7.1/userguide/variant_model.html#understanding-variant-selection)这意味着对于单个组件，该引擎可以选择不同的工件和传递性依赖性。

选择什么取决于使用者配置的属性和在生产者端找到的变体的属性。但是，某些特定的依赖关系可能会覆盖配置本身的属性。使用[Java Platform插件](https://docs.gradle.org/6.7.1/userguide/java_platform_plugin.html#java_platform_plugin)时通常是这种情况：该插件会构建一种特殊的组件，称为“平台”，可以通过将组件类别属性设置为来解决，`platform`这与以库为目标的典型依赖项相反。
...
在[复合构建中](https://docs.gradle.org/6.7.1/userguide/composite_builds.html#composite_build_intro)，将不应用您必须匹配所请求的确切依赖属性的规则：使用复合时，Gradle将自动匹配所请求的属性。换句话说，如果您包含另一个构建，则意味着您要用包含的构建中的等效变体替换替换模块的
_所有变体_ 。  
...
例如，假设您需要用其[测试治具](https://docs.gradle.org/6.7.1/userguide/java_testing.html#sec:java_test_fixtures)替代常规依赖项。您可以使用以下依赖替换规则来实现：
...
在[组合构建中](https://docs.gradle.org/6.7.1/userguide/composite_builds.html#composite_build_intro)，不应用您必须匹配确切的请求依赖功能的规则：使用组合时，Gradle将自动匹配请求的功能。换句话说，如果包含另一个构建，则意味着要用包含的构建中的等效变体替换替换模块的
_所有variants__ 。  
...
工件，通常是这种情况，但是您可能还需要选择具有不同文件类型或扩展名的工件。Gradle不鼓励在依赖项中使用分类器，而是更喜欢将工件作为[模块的其他变体](https://docs.gradle.org/6.7.1/userguide/variant_model.html#understanding-
...
与[在依赖项声明](https://docs.gradle.org/6.7.1/userguide/dependency_downgrade_and_exclude.html#sec:excluding-transitive-deps)中排除[依赖项](https://docs.gradle.org/6.7.1/userguide/dependency_downgrade_and_exclude.html#sec:excluding-transitive-deps)类似，您可以使用[Configuration.exclude（java.util.Map）](https://docs.gradle.org/6.7.1/dsl/org.gradle.api.artifacts.Configuration.html#org.gradle.api.artifacts.Configuration:exclude\(java.util.Map\))完全排除特定配置的传递性依赖项。对于配置上声明的所有依赖关系，这将自动排除传递依赖关系。
...
请转至[有关存储库内容过滤的部分，](https://docs.gradle.org/6.7.1/userguide/declaring_repositories.html#sec:repository-content-filtering)以了解有关此功能的更多信息。

## [](#sec:ivy_dynamic_resolve_mode)[启用Ivy动态解析模式](#sec:ivy_dynamic_resolve_mode)
