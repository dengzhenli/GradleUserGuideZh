...
就[功能冲突](/md/处理互斥依赖性.md#组件功能介绍)而言，最好表达模块[冲突](/md/处理互斥依赖性.md#组件功能介绍)。但是，如果没有声明这样的规则，或者您正在使用不支持功能的Gradle版本，则Gradle提供了解决这些问题的工具。
...
在[复合构建中](/md/复合构建.md#什么是复合构建？)，将不应用您必须匹配所请求的确切依赖属性的规则：使用复合时，Gradle将自动匹配所请求的属性。换句话说，如果您包含另一个构建，则意味着您要用包含的构建中的等效变体替换替换模块的
_所有变体_ 。  
...
例如，假设您需要用其[测试治具](/md/在Java和JVM项目中进行测试.md#使用测试治具)替代常规依赖项。您可以使用以下依赖替换规则来实现：
...
在[组合构建中](/md/复合构建.md#什么是复合构建？)，不应用您必须匹配确切的请求依赖功能的规则：使用组合时，Gradle将自动匹配请求的功能。换句话说，如果包含另一个构建，则意味着要用包含的构建中的等效变体替换替换模块的
_所有variants__ 。  
