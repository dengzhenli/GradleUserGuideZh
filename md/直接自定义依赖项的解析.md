...
就[功能冲突](/md/处理互斥依赖性_md#组件功能介绍)而言，最好表达模块[冲突](/md/处理互斥依赖性_md#组件功能介绍)。但是，如果没有声明这样的规则，或者您正在使用不支持功能的Gradle版本，则Gradle提供了解决这些问题的工具。
...
在[复合构建中](/md/复合构建_md#什么是复合构建？)，将不应用您必须匹配所请求的确切依赖属性的规则：使用复合时，Gradle将自动匹配所请求的属性。换句话说，如果您包含另一个构建，则意味着您要用包含的构建中的等效变体替换替换模块的
_所有变体_ 。  
...
与[属性替换](#用属性替换依赖项)类似，Gradle允许您将具有或不具有功能的依赖项替换为具有或不具有功能的另一个依赖项。

例如，假设您需要用其[测试治具](/md/在Java和JVM项目中进行测试_md#使用测试治具)替代常规依赖项。您可以使用以下依赖替换规则来实现：
...
在[组合构建中](/md/复合构建_md#什么是复合构建？)，不应用您必须匹配确切的请求依赖功能的规则：使用组合时，Gradle将自动匹配请求的功能。换句话说，如果包含另一个构建，则意味着要用包含的构建中的等效变体替换替换模块的
_所有variants__ 。  
  
╚═════════════════════════════    
  
### [用分类器或工件替换依赖项](#用分类器或工件替换依赖项)
...
与[在依赖项声明](/md/降级版本并排除依赖项_md#排除传递依赖)中排除[依赖项](/md/降级版本并排除依赖项_md#排除传递依赖)类似，您可以使用[Configuration.exclude（java.util.Map）](https://docs.gradle.org/6.7.1/dsl/org.gradle.api.artifacts.Configuration.html#org.gradle.api.artifacts.Configuration:exclude\(java.util.Map\))完全排除特定配置的传递性依赖项。对于配置上声明的所有依赖关系，这将自动排除传递依赖关系。
...
请转至[有关存储库内容过滤的部分，](/md/声明存储库_md#储存库内容过滤)以了解有关此功能的更多信息。
