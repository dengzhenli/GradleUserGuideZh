...
内容
  ...
  * [启用Ivy动态解析模式](#sec:ivy_dynamic_resolve_mode)
...
<h2 id = '#sec:dependency_resolve_rules'> <a href = '#sec:dependency_resolve_rules'>使用依赖关系解决规则</a> </h2>
...
<h3 id = '#sec:custom_versioning_scheme'> <a href = '#sec:custom_versioning_scheme'>实施自定义版本控制方案</a> </h3>
...
<h3 id = '#sec:denying_version'> <a href = '#sec:denying_version'>拒绝替换特定版本</a> </h3>
...
<h2 id = '#sec:module_replacement'> <a href = '#sec:module_replacement'>使用模块更换规则</a> </h2>

就[功能冲突](/md/处理互斥依赖性.md#sub:capabilities)而言，最好表达模块[冲突](/md/处理互斥依赖性.md#sub:capabilities)。但是，如果没有声明这样的规则，或者您正在使用不支持功能的Gradle版本，则Gradle提供了解决这些问题的工具。
...
<h2 id = '#sec:dependency_substitution_rules'> <a href = '#sec:dependency_substitution_rules'>使用依赖替换规则</a> </h2>
...
<h3 id = '#sub:module_to_project_substitution'> <a href = '#sub:module_to_project_substitution'>将外部模块依赖项替换为项目依赖项</a> </h3>
...
<h3 id = '#sub:project_to_module_substitution'> <a href = '#sub:project_to_module_substitution'>用模块替换代替项目依赖</a> </h3>
...
<h3 id = '#sub:conditional_dependency_substitution'> <a href = '#sub:conditional_dependency_substitution'>有条件地替换依赖项</a> </h3>
...
<h3 id = '#sec:variant_aware_substitutions'> <a href = '#sec:variant_aware_substitutions'>用另一个变量替换依赖</a> </h3>
...
<h4 id = '#sec:substitution_with_attributes'> <a href = '#sec:substitution_with_attributes'>用属性替换依赖项</a> </h4>
...
<h4 id = '#sec:substitution_with_capabilities'> <a href = '#sec:substitution_with_capabilities'>用具有功能的依赖项替换依赖项</a> </h4>

与[属性替换](#sec:substitution_with_attributes)类似，Gradle允许您将具有或不具有功能的依赖项替换为具有或不具有功能的另一个依赖项。

例如，假设您需要用其[测试治具](/md/在Java和JVM项目中进行测试.md#sec:java_test_fixtures)替代常规依赖项。您可以使用以下依赖替换规则来实现：
...
<h3 id = '#sec:substitution_with_classifier'> <a href = '#sec:substitution_with_classifier'>用分类器或工件替换依赖项</a> </h3>
...
<h2 id = '#sec:disabling_resolution_transitive_dependencies'> <a href = '#sec:disabling_resolution_transitive_dependencies'>禁用传递解析</a> </h2>
...
<h2 id = '#sec:configuration_defaults'> <a href = '#sec:configuration_defaults'>设置默认配置依赖项</a> </h2>
...
请转至[有关存储库内容过滤的部分，](/md/声明存储库.md#sec:repository-content-filtering)以了解有关此功能的更多信息。

<h2 id = '#sec:ivy_dynamic_resolve_mode'> <a href = '#sec:ivy_dynamic_resolve_mode'>启用Ivy动态解析模式</a> </h2>
