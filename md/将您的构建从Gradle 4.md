...
此外，Gradle还添加了一些重要的新功能和改进的功能，您应该在构建中考虑使用这些功能：
  ...
  * [Maven Publish和Ivy Publish插件](#[4_8]切换到Maven_Publish和Ivy_Publish插件)现在支持带有[签名插件的](https://docs.gradle.org/6.7.1/userguide/signing_plugin.html#signing_plugin)数字签名。
...
需要注意的其他值得注意的更改可能会破坏您的构建，包括：
  ...
  * 的[Maven中POM排除隐式通配符的表彰](#[4_8]Gradle现在可以在Maven_POM排除中使用隐式通配符)，这可能会导致依赖性被排除那些不前。
...
### [其他变化](#其他变化)
  ...
  * Gradle现在捆绑了Java 9及更高版本的[JAXB](#[5_0]Gradle现在捆绑了Java_9及更高版本的JAXB)。您可以从中删除该`--add-modules java.xml.bind`选项`org.gradle.jvmargs`（如果已设置）。
...
理想情况下，您不应该使用此软件包中的类，但是，作为快速解决方案，您可以将显式导入添加到这些类的构建脚本中。
  ...
  * 在Worker API中，[不能再设置worker的工作目录](#[5_0]_Worker_API：无法再设置工人的工作目录)。
...
### [潜在的重大变化](#潜在的重大变化)
  ...
  * [发布到AWS S3需要新权限](#[4_10]发布到AWS_S3需要新权限)。
...
## [从4_7或更早版本升级](#从4_7或更早版本升级)
  ...
  * [切换到Maven Publish和Ivy Publish插件](#[4_8]切换到Maven_Publish和Ivy_Publish插件)
...
### [潜在的重大变化](#潜在的重大变化)
  ...
  * [Gradle现在可以在Maven POM排除中使用隐式通配符](#[4_8]Gradle现在可以在Maven_POM排除中使用隐式通配符)。
...
### [潜在的重大变化](#潜在的重大变化)
  ...
  * 现在，Visual Studio集成仅为[构建中的所有组件配置一个解决方案](#[4_6]_Visual_Studio集成仅对构建的所有组件支持单个解决方案文件)。
...
### [[5_0]Gradle现在捆绑了Java_9及更高版本的JAXB](#[5_0]Gradle现在捆绑了Java_9及更高版本的JAXB)
...
### [[5_0]_Worker_API：无法再设置工人的工作目录](#[5_0]_Worker_API：无法再设置工人的工作目录)
...
### [[4_10]发布到AWS_S3需要新权限](#[4_10]发布到AWS_S3需要新权限)
...
有关更多信息，请参阅[AWS S3跨账户访问](/md/声明存储库.md#AWS_S3跨账户访问)。
...
### [[4_8]切换到Maven_Publish和Ivy_Publish插件](#[4_8]切换到Maven_Publish和Ivy_Publish插件)
...
### [[4_8]Gradle现在可以在Maven_POM排除中使用隐式通配符](#[4_8]Gradle现在可以在Maven_POM排除中使用隐式通配符)
...
### [[4_6]_Visual_Studio集成仅对构建的所有组件支持单个解决方案文件](#[4_6]_Visual_Studio集成仅对构建的所有组件支持单个解决方案文件)
