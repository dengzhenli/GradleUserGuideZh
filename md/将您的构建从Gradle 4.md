...
内容
  ...
  * [对于所有用户](#对于所有用户)
  * [从4.10及更早版本升级](#从4.10及更早版本升级)
  * [从4.9或更早版本升级](#从4.9或更早版本升级)
  * [从4.8或更早版本升级](#从4.8或更早版本升级)
  * [从4.7或更早版本升级](#从4.7或更早版本升级)
  * [从4.6及更早版本升级](#从4.6及更早版本升级)
  * [从4.5或更早版本升级](#从4.5或更早版本升级)
  * [从4.4及更早版本升级](#从4.4及更早版本升级)
  * [从4.3或更早版本升级](#从4.3或更早版本升级)
  * [从4.2或更早版本升级](#从4.2或更早版本升级)
  * [从4.1及更早版本升级](#从4.1及更早版本升级)
  * [从4.0升级](#从4.0升级)
  * [细节变化](#细节变化)
...
## [对于所有用户](#对于所有用户)
  ...
  1. 如果尚未使用最新的4.10.x版本，请阅读以下各节[，](#从4.9或更早版本升级)以帮助您将项目升级到最新的4.10.x版本。我们建议升级到最新的4.10.x版本以获取最有用的警告和弃用信息，然后再升级到5.0。避免同时升级Gradle并迁移到Kotlin DSL，以便在出现潜在问题时简化故障排除。
...
Plugin**](https://plugins.gradle.org/plugin/com.github.johnrengelman.shadow) 。
  ...
  6. 阅读“[从4.10升级”](#从4.10及更早版本升级)部分并进行任何必要的更改。

  7. 尝试使用《[故障排除指南》](/md/对构建进行故障排除.md#troubleshooting)运行项目并调试所有错误。

此外，Gradle还添加了一些重要的新功能和改进的功能，您应该在构建中考虑使用这些功能：

  * [Maven Publish和Ivy Publish插件](#[4.8]切换到Maven%20Publish和Ivy%20Publish插件)现在支持带有[签名插件的](https://docs.gradle.org/6.7.1/userguide/signing_plugin.html#signing_plugin)数字签名。

  * 在构建中使用本机[BOM导入](#[5.0]BOM导入)。

  * 用于使工作单元并行运行的[Worker API](/md/开发自定义Gradle任务类型.md#工作者API)。

  * 用于[延迟创建和配置任务的](#[4.9]考虑尝试使用惰性API进行任务创建和配置)新API ，可以显着缩短构建的配置时间。

需要注意的其他值得注意的更改可能会破坏您的构建，包括：

  * [使用POM时将编译和运行时依赖项分开](#[5.0]使用POM时，将编译和运行时依赖项分开)

  * 更改意味着您应该[配置现有任务`wrapper`和`init`任务，](#[4.8]配置现有`wrapper`和`init`任务)而不是定义自己的[任务](#[4.8]配置现有`wrapper`和`init`任务)。

  * 的[Maven中POM排除隐式通配符的表彰](#[4.8]Gradle现在可以在Maven%20POM排除中使用隐式通配符)，这可能会导致依赖性被排除那些不前。

  * 一[的方式来改变你添加Java注释处理器的一个项目](#rel4.6:annotation_processor_configuration)。

  * 命令行客户端Gradle守护程序以及所有工作程序（包括编译器和测试执行程序）的[默认内存设置](#[5.0]默认内存设置已更改)已大大减少。

  * [几个代码质量插件](#[5.0]代码质量插件的新默认版本)的[默认版本](#[5.0]代码质量插件的新默认版本)已更新。

  * [Gradle使用的](#[5.0]库升级)几个[库版本](#[5.0]库升级)已升级。
...
### [其他变化](#其他变化)
  ...
  * Gradle现在捆绑了Java 9及更高版本的[JAXB](#[5.0]Gradle现在捆绑了Java%209及更高版本的JAXB)。您可以从中删除该`--add-modules java.xml.bind`选项`org.gradle.jvmargs`（如果已设置）。
...
理想情况下，您不应该使用此软件包中的类，但是，作为快速解决方案，您可以将显式导入添加到这些类的构建脚本中。
  ...
  * [默认情况下](#[5.0]`gradlePluginPortal()`默认情况下，存储库不再查找没有POM的JAR)，`gradlePluginPortal()`存储库[不再查找没有POM的JAR](#[5.0]`gradlePluginPortal()`默认情况下，存储库不再查找没有POM的JAR)。
  ...
  * 在Worker API中，[不能再设置worker的工作目录](#[5.0]%20Worker%20API：无法再设置工人的工作目录)。

  * 与[依赖关系和版本约束](#[5.0]改进了对依赖关系和版本限制的支持)有关的行为更改可能会影响少量用户。

  * [对DefaultTask](#[5.0]对属性工厂方法的更改`DefaultTask`)上的[属性工厂方法进行](#[5.0]对属性工厂方法的更改`DefaultTask`)了一些更改，这些更改可能会影响自定义任务的创建。
...
### [潜在的重大变化](#潜在的重大变化)
  ...
  * [发布到AWS S3需要新权限](#[4.10]发布到AWS%20S3需要新权限)。
...
## [从4.8或更早版本升级](#从4.8或更早版本升级)
  ...
  * [考虑尝试使用惰性API进行任务创建和配置](#[4.9]考虑尝试使用惰性API进行任务创建和配置)
...
## [从4.7或更早版本升级](#从4.7或更早版本升级)
  ...
  * [切换到Maven Publish和Ivy Publish插件](#[4.8]切换到Maven%20Publish和Ivy%20Publish插件)

  * [将延迟配置与发布插件一起使用](#[4.8]使用延迟配置发布插件)

  * [配置现有任务`wrapper`和`init`任务，](#[4.8]配置现有`wrapper`和`init`任务)而不是定义自己的[任务](#[4.8]配置现有`wrapper`和`init`任务)
...
### [潜在的重大变化](#潜在的重大变化)
  ...
  * [Gradle现在可以在Maven POM排除中使用隐式通配符](#[4.8]Gradle现在可以在Maven%20POM排除中使用隐式通配符)。
...
除非您通过[checkstyle.configDir](https://docs.gradle.org/6.7.1/dsl/org.gradle.api.plugins.quality.CheckstyleExtension.html#org.gradle.api.plugins.quality.CheckstyleExtension:configDir)或[checkstyle.config](https://docs.gradle.org/6.7.1/dsl/org.gradle.api.plugins.quality.CheckstyleExtension.html#org.gradle.api.plugins.quality.CheckstyleExtension:config)显式配置它们的路径，否则子项目中的Checkstyle配置文件（旧的约定场所）将被忽略。
  ...
  * Gradle[普通控制台输出](#[4.7]更改了Gradle普通控制台输出的结构)的结构已更改，这可能会破坏抓取该输出的工具。

  * 与编译，链接和安装相关的许多本机任务的API[发生了重大变化](#[4.6]与编译，链接和安装相关的本机任务的API更改)。
...
### [潜在的重大变化](#潜在的重大变化)
  ...
  * 现在，Visual Studio集成仅为[构建中的所有组件配置一个解决方案](#[4.6]%20Visual%20Studio集成仅对构建的所有组件支持单个解决方案文件)。
...
### [潜在的重大变化](#潜在的重大变化)
  ...
  * [`HttpBuildCache`不再遵循重定向](#[4.5]`HttpBuildCache`不再遵循重定向)。
...
Studio版本的安装目录来绕过工具链发现。
  ...
  * [Gradle使用的一些第三方库已升级，](#[4.4]第三方依赖项升级)可以解决安全问题。
...
### [[5.0]代码质量插件的新默认版本](#[5.0]代码质量插件的新默认版本)
...
### [[5.0]改进了对依赖关系和版本限制的支持](#[5.0]改进了对依赖关系和版本限制的支持)
...
### [[5.0]使用POM时，将编译和运行时依赖项分开](#[5.0]使用POM时，将编译和运行时依赖项分开)
...
### [[5.0]对属性工厂方法的更改`DefaultTask`](#[5.0]对属性工厂方法的更改`DefaultTask`)
...
### [[5.0]Gradle现在捆绑了Java 9及更高版本的JAXB](#[5.0]Gradle现在捆绑了Java%209及更高版本的JAXB)
...
### [[5.0]`gradlePluginPortal()`默认情况下，存储库不再查找没有POM的JAR](#[5.0]`gradlePluginPortal()`默认情况下，存储库不再查找没有POM的JAR)
...
### [[5.0] Worker API：无法再设置工人的工作目录](#[5.0]%20Worker%20API：无法再设置工人的工作目录)
...
### [[4.10]发布到AWS S3需要新权限](#[4.10]发布到AWS%20S3需要新权限)
...
### [[4.9]考虑尝试使用惰性API进行任务创建和配置](#[4.9]考虑尝试使用惰性API进行任务创建和配置)
...
### [[4.8]切换到Maven Publish和Ivy Publish插件](#[4.8]切换到Maven%20Publish和Ivy%20Publish插件)
...
### [[4.8]配置现有`wrapper`和`init`任务](#[4.8]配置现有`wrapper`和`init`任务)
...
### [[4.8]Gradle现在可以在Maven POM排除中使用隐式通配符](#[4.8]Gradle现在可以在Maven%20POM排除中使用隐式通配符)
...
### [[4.7]更改了Gradle普通控制台输出的结构](#[4.7]更改了Gradle普通控制台输出的结构)
...
### [[4.6]与编译，链接和安装相关的本机任务的API更改](#[4.6]与编译，链接和安装相关的本机任务的API更改)
...
### [[4.6] Visual Studio集成仅对构建的所有组件支持单个解决方案文件](#[4.6]%20Visual%20Studio集成仅对构建的所有组件支持单个解决方案文件)
