...
内容
  ...
  * [对于所有用户](#对于所有用户)
  * [从4.10及更早版本升级](#从4_10及更早版本升级)
  * [从4.9或更早版本升级](#从4_9或更早版本升级)
  * [从4.8或更早版本升级](#从4_8或更早版本升级)
  * [从4.7或更早版本升级](#从4_7或更早版本升级)
  * [从4.6及更早版本升级](#从4_6及更早版本升级)
  * [从4.5或更早版本升级](#从4_5或更早版本升级)
  * [从4.4及更早版本升级](#从4_4及更早版本升级)
  * [从4.3或更早版本升级](#从4_3或更早版本升级)
  * [从4.2或更早版本升级](#从4_2或更早版本升级)
  * [从4.1及更早版本升级](#从4_1及更早版本升级)
  * [从4.0升级](#从4_0升级)
  * [细节变化](#细节变化)
...
## [对于所有用户](#对于所有用户)
  ...
  1. 如果尚未使用最新的4.10.x版本，请阅读以下各节[，](#从4_9或更早版本升级)以帮助您将项目升级到最新的4.10.x版本。我们建议升级到最新的4.10.x版本以获取最有用的警告和弃用信息，然后再升级到5.0。避免同时升级Gradle并迁移到Kotlin DSL，以便在出现潜在问题时简化故障排除。
...
Plugin**](https://plugins.gradle.org/plugin/com.github.johnrengelman.shadow) 。
  ...
  6. 阅读“[从4.10升级”](#从4_10及更早版本升级)部分并进行任何必要的更改。

  7. 尝试使用《[故障排除指南》](/md/对构建进行故障排除.md#troubleshooting)运行项目并调试所有错误。

此外，Gradle还添加了一些重要的新功能和改进的功能，您应该在构建中考虑使用这些功能：

  * [Maven Publish和Ivy Publish插件](#[4_8]切换到Maven%20Publish和Ivy%20Publish插件)现在支持带有[签名插件的](https://docs.gradle.org/6.7.1/userguide/signing_plugin.html#signing_plugin)数字签名。

  * 在构建中使用本机[BOM导入](#[5_0]BOM导入)。

  * 用于使工作单元并行运行的[Worker API](/md/开发自定义Gradle任务类型_md#工作者API)。

  * 用于[延迟创建和配置任务的](#[4_9]考虑尝试使用惰性API进行任务创建和配置)新API ，可以显着缩短构建的配置时间。

需要注意的其他值得注意的更改可能会破坏您的构建，包括：

  * [使用POM时将编译和运行时依赖项分开](#[5_0]使用POM时，将编译和运行时依赖项分开)

  * 更改意味着您应该[配置现有任务`wrapper`和`init`任务，](#[4_8]配置现有`wrapper`和`init`任务)而不是定义自己的[任务](#[4_8]配置现有`wrapper`和`init`任务)。

  * 的[Maven中POM排除隐式通配符的表彰](#[4_8]Gradle现在可以在Maven%20POM排除中使用隐式通配符)，这可能会导致依赖性被排除那些不前。

  * 一[的方式来改变你添加Java注释处理器的一个项目](#rel4.6:annotation_processor_configuration)。

  * 命令行客户端Gradle守护程序以及所有工作程序（包括编译器和测试执行程序）的[默认内存设置](#[5_0]默认内存设置已更改)已大大减少。

  * [几个代码质量插件](#[5_0]代码质量插件的新默认版本)的[默认版本](#[5_0]代码质量插件的新默认版本)已更新。

  * [Gradle使用的](#[5_0]库升级)几个[库版本](#[5_0]库升级)已升级。

## [从4_10及更早版本升级](#从4_10及更早版本升级)
...
### [其他变化](#其他变化)
  ...
  * Gradle现在捆绑了Java 9及更高版本的[JAXB](#[5_0]Gradle现在捆绑了Java%209及更高版本的JAXB)。您可以从中删除该`--add-modules java.xml.bind`选项`org.gradle.jvmargs`（如果已设置）。
...
在应用Java插件时，其行为略有不同（例如，它添加了`api`配置）。因此，请确保在升级后检查您的构建是否符合预期。
  ...
  * 该[配置避免API](#配置回避API禁止常见的配置错误)已更新，以防止从未使用任务的创建和配置。
...
运行Gradle和构建环境
  ...
  * 如前所述，Gradle不能再在Java 7上运行。但是，您仍然可以使用[派生的编译和测试](/md/构建Java和JVM项目_md#定位特定的Java版本)来构建和测试Java 6及更高版本的软件。

  * 该`-Dtest.single`命令行选项已被删除-使用[测试过滤](/md/在Java和JVM项目中进行测试_md#测试过滤)来代替。

  * 该`-Dtest.debug`命令行选项已被删除-使用[`--debug-jvm`选项](/md/在Java和JVM项目中进行测试_md#运行测试时进行调试)来代替。
...
任务和属性
  ...
  * 删除了以下与[惰性属性](/md/延迟配置_md#惰性集合)相关的旧类和方法-使用[ObjectFactory.property（）](https://docs.gradle.org/6.7.1/javadoc/org/gradle/api/model/ObjectFactory.html#property-java.lang.Class-)创建`Property`实例：
  ...
  * 该`Task.dependsOnTaskDidWork()`方法已删除-改用[声明的输入和输出](/md/处理任务_md#最新检查（又称增量构建）)。

  * `TaskInternal`已删除以下属性和方法-使用任务依赖项，任务规则，可重用的实用程序方法或[Worker API](/md/开发自定义Gradle任务类型_md#工作者API)代替直接执行任务。
  ...
  * 现在，覆盖[4.8](#弃用)中[弃用的](#弃用)内置任务会产生错误。
...
理想情况下，您不应该使用此软件包中的类，但是，作为快速解决方案，您可以将显式导入添加到这些类的构建脚本中。
  ...
  * [默认情况下](#[5_0]`gradlePluginPortal()`默认情况下，存储库不再查找没有POM的JAR)，`gradlePluginPortal()`存储库[不再查找没有POM的JAR](#[5_0]`gradlePluginPortal()`默认情况下，存储库不再查找没有POM的JAR)。
  ...
  * 在Worker API中，[不能再设置worker的工作目录](#[5_0]%20Worker%20API：无法再设置工人的工作目录)。

  * 与[依赖关系和版本约束](#[5_0]改进了对依赖关系和版本限制的支持)有关的行为更改可能会影响少量用户。

  * [对DefaultTask](#[5_0]对属性工厂方法的更改`DefaultTask`)上的[属性工厂方法进行](#[5_0]对属性工厂方法的更改`DefaultTask`)了一些更改，这些更改可能会影响自定义任务的创建。

## [从4_9或更早版本升级](#从4_9或更早版本升级)

如果您尚未安装4.9版，请跳至适用于当前Gradle版本的部分，然后逐步升级，直至到达此处。然后，在升级到Gradle 4.10时应用这些更改。

### [不推荐使用的类，方法和属性](#不推荐使用的类，方法和属性)
...
### [潜在的重大变化](#潜在的重大变化)
  ...
  * [发布到AWS S3需要新权限](#[4_10]发布到AWS%20S3需要新权限)。
...
## [从4_8或更早版本升级](#从4_8或更早版本升级)

  * [考虑尝试使用惰性API进行任务创建和配置](#[4_9]考虑尝试使用惰性API进行任务创建和配置)
...
## [从4_7或更早版本升级](#从4_7或更早版本升级)

  * [切换到Maven Publish和Ivy Publish插件](#[4_8]切换到Maven%20Publish和Ivy%20Publish插件)

  * [将延迟配置与发布插件一起使用](#[4_8]使用延迟配置发布插件)

  * [配置现有任务`wrapper`和`init`任务，](#[4_8]配置现有`wrapper`和`init`任务)而不是定义自己的[任务](#[4_8]配置现有`wrapper`和`init`任务)
...
### [潜在的重大变化](#潜在的重大变化)
  ...
  * [Gradle现在可以在Maven POM排除中使用隐式通配符](#[4_8]Gradle现在可以在Maven%20POM排除中使用隐式通配符)。
...
除非您通过[checkstyle.configDir](https://docs.gradle.org/6.7.1/dsl/org.gradle.api.plugins.quality.CheckstyleExtension.html#org.gradle.api.plugins.quality.CheckstyleExtension:configDir)或[checkstyle.config](https://docs.gradle.org/6.7.1/dsl/org.gradle.api.plugins.quality.CheckstyleExtension.html#org.gradle.api.plugins.quality.CheckstyleExtension:config)显式配置它们的路径，否则子项目中的Checkstyle配置文件（旧的约定场所）将被忽略。
  ...
  * Gradle[普通控制台输出](#[4_7]更改了Gradle普通控制台输出的结构)的结构已更改，这可能会破坏抓取该输出的工具。

  * 与编译，链接和安装相关的许多本机任务的API[发生了重大变化](#[4_6]与编译，链接和安装相关的本机任务的API更改)。
...
### [潜在的重大变化](#潜在的重大变化)
  ...
  * 现在，Visual Studio集成仅为[构建中的所有组件配置一个解决方案](#[4_6]%20Visual%20Studio集成仅对构建的所有组件支持单个解决方案文件)。
...
### [潜在的重大变化](#潜在的重大变化)
  ...
  * [`HttpBuildCache`不再遵循重定向](#[4_5]`HttpBuildCache`不再遵循重定向)。
...
Studio版本的安装目录来绕过工具链发现。
  ...
  * [Gradle使用的一些第三方库已升级，](#[4_4]第三方依赖项升级)可以解决安全问题。

## [从4_2或更早版本升级](#从4_2或更早版本升级)

  * `plugins {}`现在，该块可以[用于子项目中](/md/使用Gradle插件_md#将具有相同版本的外部插件应用于子项目)，也可以[用于](/md/使用Gradle插件_md#将具有相同版本的外部插件应用于子项目)[ _buildSrc_](/md/使用Gradle插件_md#从_buildSrc_目录应用插件)[ 目录中的](/md/使用Gradle插件_md#从_buildSrc_目录应用插件)[插件 __](/md/使用Gradle插件_md#从_buildSrc_目录应用插件)。
...
## [从4_0升级](#从4_0升级)
  ...
  * 考虑使用新的[Worker API](/md/开发自定义Gradle任务类型_md#工作者API)来使构建中的工作单元并行运行。

### [不推荐使用的类，方法和属性](#不推荐使用的类，方法和属性_2)
...
### [潜在的重大变化](#潜在的重大变化)
  ...
  * 与Java项目有[项目依赖性](/md/声明依赖_md#项目依赖)的非Java项目现在`runtimeElements`默认使用配置而不是`default`配置。
...
### [[5_0]默认内存设置已更改](#[5_0]默认内存设置已更改)

现在，命令行客户端从64MB的堆开始，而不是1GB。这可能会影响使用`--no-
daemon`模式在客户端VM内部直接运行的内部版本。我们不鼓励使用`--no-
daemon`，但是如果必须使用它，则可以使用`GRADLE_OPTS`环境变量来增加可用内存。

Gradle守护程序现在从512MB的堆而不是1GB的堆开始。大型项目可能必须使用该[`org.gradle.jvmargs`](/md/Gradle环境搭建_md#配置JVM内存)属性来增加此设置。

现在，所有工作程序（包括编译器和测试执行程序）都以512MB的堆开始。先前的默认值为物理内存的1/4。大型项目可能必须在相关任务上增加此设置，例如[`JavaCompile`](/md/Gradle环境搭建_md#配置JVM内存)或[`Test`](https://docs.gradle.org/6.7.1/dsl/org.gradle.api.tasks.testing.Test.html)。

### [[5_0]代码质量插件的新默认版本](#[5_0]代码质量插件的新默认版本)
...
### [[5_0]改进了对依赖关系和版本限制的支持](#[5_0]改进了对依赖关系和版本限制的支持)
...
### [[5_0]使用POM时，将编译和运行时依赖项分开](#[5_0]使用POM时，将编译和运行时依赖项分开)
...
通过这种新行为，Java和Java库插件都可以[实现编译范围和运行时范围](/md/Java库插件_md#API与实现分离)的[分离](/md/Java库插件_md#API与实现分离)。这意味着编译类路径仅包含编译范围的依赖关系，而运行时类路径也添加了运行时范围的依赖关系。如果您使用Gradle开发和发布Java库，并且它们之间的依赖关系`api`和`implementation`依赖关系反映在已发布的范围中，则这特别有用。

### [[5_0]对属性工厂方法的更改`DefaultTask`](#[5_0]对属性工厂方法的更改`DefaultTask`)

#### [物业工厂方法`DefaultTask`现已确定](#物业工厂方法`DefaultTask`现已确定)

诸如此类的属性工厂方法`newInputFile()`旨在从extended类型的构造函数中调用`DefaultTask`。这些方法现在是最终方法，以避免子类覆盖这些方法并使用未初始化的状态。

#### [输入和输出不会自动注册](#输入和输出不会自动注册)
...
### [[5_0]Gradle现在捆绑了Java%209及更高版本的JAXB](#[5_0]Gradle现在捆绑了Java%209及更高版本的JAXB)
...
### [[5_0]`gradlePluginPortal()`默认情况下，存储库不再查找没有POM的JAR](#[5_0]`gradlePluginPortal()`默认情况下，存储库不再查找没有POM的JAR)
...
### [Java库分发插件利用Java库插件](#Java库分发插件利用Java库插件)
...
### [配置回避API禁止常见的配置错误](#配置回避API禁止常见的配置错误)
...
### [[5_0]%20Worker%20API：无法再设置工人的工作目录](#[5_0]%20Worker%20API：无法再设置工人的工作目录)
...
### [[4_10]发布到AWS%20S3需要新权限](#[4_10]发布到AWS%20S3需要新权限)
...
有关更多信息，请参阅[AWS S3跨账户访问](/md/声明存储库_md#AWS%20S3跨账户访问)。

### [[4_9]考虑尝试使用惰性API进行任务创建和配置](#[4_9]考虑尝试使用惰性API进行任务创建和配置)
...
### [[4_8]切换到Maven%20Publish和Ivy%20Publish插件](#[4_8]切换到Maven%20Publish和Ivy%20Publish插件)
...
### [[4_8]配置现有`wrapper`和`init`任务](#[4_8]配置现有`wrapper`和`init`任务)
...
### [[4_8]Gradle现在可以在Maven%20POM排除中使用隐式通配符](#[4_8]Gradle现在可以在Maven%20POM排除中使用隐式通配符)
...
### [[4_7]更改了Gradle普通控制台输出的结构](#[4_7]更改了Gradle普通控制台输出的结构)
...
### [[4_6]与编译，链接和安装相关的本机任务的API更改](#[4_6]与编译，链接和安装相关的本机任务的API更改)
...
### [[4_6]%20Visual%20Studio集成仅对构建的所有组件支持单个解决方案文件](#[4_6]%20Visual%20Studio集成仅对构建的所有组件支持单个解决方案文件)
