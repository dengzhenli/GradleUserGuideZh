...
与[发布解析版本](/md/Maven发布插件.md#自定义依赖版本)结合使用，您还可以在发布时替换声明的动态版本部件。消费者将看到您的发行版已解决的版本。

每个[依赖项配置](/md/声明依赖.md#什么是依赖项配置)均启用锁定。启用后，您必须创建一个初始锁定状态。这将使Gradle验证分辨率结果没有改变，即使生成了新版本，也会导致选择的依赖项相同。对构建的修改会影响已解决的依赖关系集，将导致其失败。这样可以确保发布的依赖项或构建定义中的更改不会在不调整锁定状态的情况下更改分辨率。

╔═════════════════════════════  

依赖锁定仅在[动态版本中](/md/处理随时间变化的版本.md#声明动态版本)才有意义。尽管内容可能会更改，但对坐标不变的[版本](/md/处理随时间变化的版本.md#声明版本变更)（如`-SNAPSHOT`）不会产生影响。当持久锁状态和解决结果中存在变化的依赖性时，Gradle甚至会发出警告。  
...
如果将插件应用于构建，则可能还需要利用依赖锁定。为了锁定用于脚本插件的[`classpath`配置](/md/使用Gradle插件.md#使用带有buildscript块的插件)，请执行以下操作：
...
locks`项目或子项目目录内的文件夹中的文件中。每个文件均由其锁定的配置命名，并具有`lockfile`扩展名。该规则的一个例外是构建[脚本本身的](/md/使用Gradle插件.md#使用带有buildscript块的插件)配置。在这种情况下，配置名称将带有前缀`buildscript-`。
