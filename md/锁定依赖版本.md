...
与[发布解析版本](/md/Maven发布插件.md#publishing_maven:resolved_dependencies)结合使用，您还可以在发布时替换声明的动态版本部件。消费者将看到您的发行版已解决的版本。

每个[依赖项配置](/md/声明依赖.md#sec:what-are-dependency-configurations)均启用锁定。启用后，您必须创建一个初始锁定状态。这将使Gradle验证分辨率结果没有改变，即使生成了新版本，也会导致选择的依赖项相同。对构建的修改会影响已解决的依赖关系集，将导致其失败。这样可以确保发布的依赖项或构建定义中的更改不会在不调整锁定状态的情况下更改分辨率。

╔═════════════════════════════  

依赖锁定仅在[动态版本中](/md/处理随时间变化的版本.md#sub:declaring_dependency_with_dynamic_version)才有意义。尽管内容可能会更改，但对坐标不变的[版本](/md/处理随时间变化的版本.md#sub:declaring_dependency_with_changing_version)（如`-SNAPSHOT`）不会产生影响。当持久锁状态和解决结果中存在变化的依赖性时，Gradle甚至会发出警告。  
  
╚═════════════════════════════    
  
<h2 id = '#enabling_locking_on_configurations'> <a href = '#enabling_locking_on_configurations'>启用配置锁定</a> </h2>
...
<h3 id = '#locking_buildscript_classpath_configuration'> <a href = '#locking_buildscript_classpath_configuration'>锁定buildscript类路径配置</a> </h3>

如果将插件应用于构建，则可能还需要利用依赖锁定。为了锁定用于脚本插件的[`classpath`配置](/md/使用Gradle插件.md#sec:applying_plugins_buildscript)，请执行以下操作：
...
<h2 id = '#generating_and_updating_dependency_locks'> <a href = '#generating_and_updating_dependency_locks'>生成和更新依赖关系锁</a> </h2>

为了生成或更新锁定状态，`--write-
locks`除了将触发要解析的配置的常规任务之外，还指定命令行参数。这将导致在该构建执行中为每个已解析的配置创建锁定状态。请注意，如果先前存在锁定状态，则将其覆盖。

<h3 id = '#lock_all_configurations_in_one_build_execution'> <a href = '#lock_all_configurations_in_one_build_execution'>将所有配置锁定在一个构建执行中</a> </h3>
...
<h2 id = '#lock_state_location_and_format'> <a href = '#lock_state_location_and_format'>锁定状态的位置和格式</a> </h2>

锁定状态将保留在`gradle/dependency-
locks`项目或子项目目录内的文件夹中的文件中。每个文件均由其锁定的配置命名，并具有`lockfile`扩展名。该规则的一个例外是构建[脚本本身的](/md/使用Gradle插件.md#sec:applying_plugins_buildscript)配置。在这种情况下，配置名称将带有前缀`buildscript-`。
...
<h2 id = '#running_a_build_with_lock_state_present'> <a href = '#running_a_build_with_lock_state_present'>在存在锁定状态的情况下运行构建</a> </h2>
...
<h3 id = '#fine_tuning_dependency_locking_behaviour_with_lock_mode'> <a href = '#fine_tuning_dependency_locking_behaviour_with_lock_mode'>使用锁定模式微调依赖项锁定行为</a> </h3>
...
<h2 id = '#selectively_updating_lock_state_entries'> <a href = '#selectively_updating_lock_state_entries'>有选择地更新锁定状态条目</a> </h2>
...
<h2 id = '#disabling_dependency_locking'> <a href = '#disabling_dependency_locking'>禁用依赖项锁定</a> </h2>
...
<h2 id = '#single_lock_file_per_project'> <a href = '#single_lock_file_per_project'>每个项目一个锁文件</a> </h2>
...
可以通过启用匹配[功能预览](/md/功能生命周期.md#feature_preview)来激活格式：
...
<h3 id = '#migrating_to_the_single_lockfile_per_project_format'> <a href = '#migrating_to_the_single_lockfile_per_project_format'>迁移到每个项目格式的单个锁定文件</a> </h3>
...
<h3 id = '#configuring_the_per_project_lock_file_name_and_location'> <a href = '#configuring_the_per_project_lock_file_name_and_location'>配置每个项目的锁定文件名和位置</a> </h3>
...
<h2 id = '#ignoring_dependencies'> <a href = '#ignoring_dependencies'>从锁定状态忽略特定的依赖关系</a> </h2>
...
<h2 id = '#locking_limitations'> <a href = '#locking_limitations'>锁定限制</a> </h2>

  * 锁定尚不能应用于源依赖项。

<h2 id = '#nebula_locking_plugin'> <a href = '#nebula_locking_plugin'>Nebula 锁定插件</a> </h2>
