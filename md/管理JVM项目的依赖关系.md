...
本章说明如何将基本的依赖管理概念应用于基于JVM的项目。有关依赖管理的详细介绍，请参见[Gradle中的依赖管理](https://docs.gradle.org/6.7.1/userguide/core_dependency_management.html)。

## [](#sec:setting_up_a_standard_build_script_java_tutorial)[剖析典型的构建脚本](#sec:setting_up_a_standard_build_script_java_tutorial)

让我们看一下基于JVM的项目的非常简单的构建脚本。它应用了[Java库插件](https://docs.gradle.org/6.7.1/userguide/java_library_plugin.html#java_library_plugin)，该[插件](https://docs.gradle.org/6.7.1/userguide/java_library_plugin.html#java_library_plugin)会自动引入标准项目布局，提供执行典型工作的任务以及对依赖项管理的充分支持。
...
有多种[类型的依赖关系](https://docs.gradle.org/6.7.1/userguide/declaring_dependencies.html#sec:dependency-
types)，你可以声明。一种这样的类型是 _模块依赖性_
。一个[模块依赖](https://docs.gradle.org/6.7.1/userguide/declaring_dependencies.html#sub:module_dependencies)代表与当前构建建外特定版本模块的依赖关系。模块通常存储在存储库中，例如Maven
...
要了解有关定义依赖项的更多信息，请[参阅声明依赖项](https://docs.gradle.org/6.7.1/userguide/declaring_dependencies.html)。
...
考虑到这三个目的，让我们看一下[Java库插件定义](https://docs.gradle.org/6.7.1/userguide/java_library_plugin.html#sec:java_library_configurations_graph)的一些[标准配置](https://docs.gradle.org/6.7.1/userguide/java_library_plugin.html#sec:java_library_configurations_graph)。
...
各种插件添加了进一步的标准配置。您还可以通过[Project.configurations{}](https://docs.gradle.org/6.7.1/dsl/org.gradle.api.Project.html#org.gradle.api.Project:configurations\(groovy.lang.Closure\))在构建中定义自己的自定义配置。有关定义和自定义依赖项配置的详细信息，请参阅[什么是](https://docs.gradle.org/6.7.1/userguide/declaring_dependencies.html#sec:what-are-dependency-configurations)依赖项配置。

## [](#sec:repositories_java_tutorial)[声明通用Java存储库](#sec:repositories_java_tutorial)

Gradle如何知道在哪里可以找到外部依赖文件？Gradle在 _存储库中_
查找它们。库是模块，通过组织的集合`group`，`name`和`version`。Gradle了解不同的[存储库类型](https://docs.gradle.org/6.7.1/userguide/declaring_repositories.html#sec:repository-
...
要了解有关定义存储库的更多信息，请[参阅声明存储库](https://docs.gradle.org/6.7.1/userguide/declaring_repositories.html)。

## [](#sec:publishing_artifacts_java_tutorial)[发布工件](#sec:publishing_artifacts_java_tutorial)

要了解有关发布工件的更多信息，请查看[发布插件](https://docs.gradle.org/6.7.1/userguide/publishing_setup.html)。
