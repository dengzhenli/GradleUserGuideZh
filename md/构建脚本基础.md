...
内容
  ...
  * [动态任务](#动态任务)
  * [处理现有任务](#处理现有任务)
  * [Groovy DSL快捷方式符号](#Groovy%20DSL快捷方式符号)
  * [额外任务属性](#额外任务属性)
  * [使用Ant任务](#使用Ant任务)
  * [使用方法](#使用方法)
  * [默认任务](#默认任务)
...
可以在定义之前声明`taskX`to的依赖项。对于多项目构建，这种自由非常重要。[将依赖项添加到任务](/md/处理任务_md#向任务添加依赖项)中将详细讨论任务依赖[项](/md/处理任务_md#向任务添加依赖项)。`taskY``taskY`[](/md/处理任务_md#向任务添加依赖项)

请注意，在引用尚未定义的任务时，不能使用[快捷方式表示法](#Groovy%20DSL快捷方式符号)。
...
## [Groovy%20DSL快捷方式符号](#Groovy%20DSL快捷方式符号)
...
额外的属性不仅限于任务。您可以在[Extra属性中](/md/编写构建脚本_md#额外的属性)阅读有关它们的更多信息。
