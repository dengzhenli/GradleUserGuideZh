...
在添加对表示依赖项替换中的[变体支持的支持](/md/直接自定义依赖项的解析_md#用另一个变量替换依赖)时，错误修复引入了某些构建可能依赖的行为更改。以前，替换的依赖项仍将使用原始选择器的[属性](/md/了解变体选择.md)，而不是替换选择器的[属性](/md/了解变体选择.md)。
...
如果您受到影响，请参阅[文档](/md/直接自定义依赖项的解析_md#用另一个变量替换依赖)以解决问题。
...
使用Gradle
6.4，用于[依赖项锁定`LockMode`](/md/锁定依赖版本_md#使用锁定模式微调依赖项锁定行为)的孵化API已更改。现在通过a设置值，`Property<LockMode>`而不是直接设置器。这意味着必须为Kotlin
DSL更新设置值的表示法：
...
6.3不支持[丰富的控制台](/md/命令行界面_md#丰富控制台)。Microsoft
Windows 32位不受影响。
...
配置都是可以解决的。要声明依赖关系，请使用您使用的插件提供的配置，例如[Java库插件](/md/Java库插件_md#Java库插件配置)。
...
现在，JVM项目中的类路径显式请求该`org.gradle.category=library`属性。如果无法使用某些库，这将导致更清晰的错误消息。例如，当库不支持所需的Java版本时。实际的效果是，现在所有[平台依赖项](/md/Java平台插件_md#消费平台)都必须这样声明。以前，当`platform()`本地平台或使用Gradle
...
#### [`ProjectLayout`%20不再可用于工作者作为服务的行为](#`ProjectLayout`%20不再可用于工作者作为服务的行为)
...
#### [发布Spring%20Boot应用程序](#发布Spring%20Boot应用程序)
