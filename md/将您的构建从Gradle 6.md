...
4.x或5.x进行迁移，请首先参阅[旧的迁移指南](https://docs.gradle.org/6.7.1/userguide/upgrading_version_5.html#upgrading_version_5)。
...
实际上与Ant相同。有关更多信息，请参见[用户手册](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:file_trees)。有时，Ant的默认排除会证明是有问题的，例如，当您要`.gitignore`在存档文件中包含时。

在执行阶段更改Gradle的默认排除项可能导致最新检查的正确性问题，因此不建议使用。仅允许您在设置脚本中更改Gradle的默认排除项，有关示例，请参见[用户手册](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:change_default_excludes)。
...
在添加对表示依赖项替换中的[变体支持的支持](https://docs.gradle.org/6.7.1/userguide/resolution_rules.html#sec:variant_aware_substitutions)时，错误修复引入了某些构建可能依赖的行为更改。以前，替换的依赖项仍将使用原始选择器的[属性](https://docs.gradle.org/6.7.1/userguide/variant_attributes.html)，而不是替换选择器的[属性](https://docs.gradle.org/6.7.1/userguide/variant_attributes.html)。
...
如果发生以下情况，您可能会受到此更改的影响：
  ...
  * _并_ 在这些依赖项上使用[解析规则](https://docs.gradle.org/6.7.1/userguide/resolution_rules.html)。

如果您受到影响，请参阅[文档](https://docs.gradle.org/6.7.1/userguide/resolution_rules.html#sec:variant_aware_substitutions)以解决问题。
...
#### [](#upgrade:pmd_expects_6)[PMD插件默认需要PMD
6.0.0或更高版本](#upgrade:pmd_expects_6)
...
使用Gradle
6.4，用于[依赖项锁定`LockMode`](https://docs.gradle.org/6.7.1/userguide/dependency_locking.html#fine_tuning_dependency_locking_behaviour_with_lock_mode)的孵化API已更改。现在通过a设置值，`Property<LockMode>`而不是直接设置器。这意味着必须为Kotlin
DSL更新设置值的表示法：
...
#### [](#ivy_repositories_with_custom_layouts)[具有自定义布局的Ivy存储库](#ivy_repositories_with_custom_layouts)
...
6.3不支持[丰富的控制台](https://docs.gradle.org/6.7.1/userguide/command_line_interface.html#sec:rich_console)。Microsoft
Windows 32位不受影响。
...
插件添加的。这些配置不再用于使用[变体感知依赖性管理](https://docs.gradle.org/6.7.1/userguide/variant_model.html)和[新发布插件的](https://docs.gradle.org/6.7.1/userguide/publishing_setup.html)现代Gradle构建中。

虽然目前这些配置仍保留在Gradle中以实现向后兼容，但现在不建议使用它们来声明依赖项或解析依赖项。

解决这些配置绝不是预期的用例，仅是可能的，因为在较早的Gradle版本中， _每个_
配置都是可以解决的。要声明依赖关系，请使用您使用的插件提供的配置，例如[Java库插件](https://docs.gradle.org/6.7.1/userguide/java_library_plugin.html#sec:java_library_configurations_graph)。
...
现在，JVM项目中的类路径显式请求该`org.gradle.category=library`属性。如果无法使用某些库，这将导致更清晰的错误消息。例如，当库不支持所需的Java版本时。实际的效果是，现在所有[平台依赖项](https://docs.gradle.org/6.7.1/userguide/java_platform_plugin.html#sec:java_platform_consumption)都必须这样声明。以前，当`platform()`本地平台或使用Gradle
...
suite`）请查阅文档[构建Java和JVM的项目](https://docs.gradle.org/6.7.1/userguide/building_java_projects.html)，以确定其稳定的JVM插件是适合您的项目。

### [](#potential_breaking_changes_7)[潜在的重大变化](#potential_breaking_changes_7)

#### [](#projectlayout_is_no_longer_available_to_worker_actions_as_a_service)[`ProjectLayout`
不再可用于工作者作为服务的行为](#projectlayout_is_no_longer_available_to_worker_actions_as_a_service)
