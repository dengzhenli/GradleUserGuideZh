...
#### [PMD插件默认需要PMD6.0.0或更高版本](#PMD插件默认需要PMD6.0.0或更高版本)
...
#### [项目根目录中的属性`gradle.properties`泄漏到`buildSrc`包含的构建中](#项目根目录中的属性`gradle.properties`泄漏到`buildSrc`包含的构建中)
...
#### [`ProjectLayout` 不再可用于工作者作为服务的行为](#`ProjectLayout`%20不再可用于工作者作为服务的行为)
...
#### [发布Spring Boot应用程序](#发布Spring%20Boot应用程序)
