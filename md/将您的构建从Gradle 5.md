...
内容
  ...
  * [从5.6及更早版本升级](#从5.6及更早版本升级)
  * [从5.5或更早版本升级](#从5.5或更早版本升级)
  * [从5.4或更早版本升级](#从5.4或更早版本升级)
  * [从5.3或更早版本升级](#从5.3或更早版本升级)
  * [从5.2或更早版本升级](#从5.2或更早版本升级)
  * [从5.1或更早版本升级](#从5.1或更早版本升级)
  * [从5.0或更早版本升级](#从5.0或更早版本升级)
...
#### [不再应该使用`compile`和`runtime`配置声明依赖项](#不再应该使用`compile`和`runtime`配置声明依赖项)
...
#### [`StartParameter`不建议使用向上搜索相关的API](#`StartParameter`不建议使用向上搜索相关的API)

在Gradle 5.0中，我们删除了`--no-search-upward`CLI参数。

`StartParameter`（如`isSearchUpwards()`）中的相关API现在已弃用。

#### [API`BuildListener.buildStarted`和`Gradle.buildStarted`已被弃用](#API`BuildListener.buildStarted`和`Gradle.buildStarted`已被弃用)

这些方法当前无法按预期方式工作，因为在构建开始之后将永远不会调用回调。

不推荐使用这些方法以避免混淆。

#### [不建议使用隐式重复策略`Copy`或存档任务](#不建议使用隐式重复策略`Copy`或存档任务)
...
#### [在不使用设置文件的情况下执行Gradle已被弃用](#在不使用设置文件的情况下执行Gradle已被弃用)
...
#### [不建议调用`Project.afterEvaluate`评估项目](#不建议调用`Project.afterEvaluate`评估项目)
...
#### [不再支持Android Gradle Plugin 3.3和更早版本](#不再支持Android%20Gradle%20Plugin%203.3和更早版本)

Gradle 6.0支持Android Gradle插件版本3.4及更高版本。

#### [不再支持构建扫描插件2.x](#不再支持构建扫描插件2.x)
...
#### [复合构建中对构建和任务名称的更改](#复合构建中对构建和任务名称的更改)
...
#### [现在，将buildSrc保留为项目和子项目的内部版本名称](#现在，将buildSrc保留为项目和子项目的内部版本名称)
...
#### [无法打包或解包缓存的结果现在将使构建失败](#无法打包或解包缓存的结果现在将使构建失败)
...
#### [buildSrc项目自动使用构建缓存配置](#buildSrc项目自动使用构建缓存配置)

以前，为了将构建缓存用于buildSrc构建，您需要在buildSrc构建中复制构建缓存配置。现在，它会自动使用顶级设置脚本定义的构建缓存配置。

#### [依赖管理的变化](#依赖管理的变化)

#### [Gradle模块元数据总是发布](#Gradle模块元数据总是发布)
...
#### [Gradle模块元数据具有更严格的验证](#Gradle模块元数据具有更严格的验证)
...
#### [默认情况下，不再向Maven或Ivy存储库查询没有元数据的工件](#默认情况下，不再向Maven或Ivy存储库查询没有元数据的工件)
...
#### [`ivy.xml`Java库的出版物包含更多信息](#`ivy.xml`Java库的出版物包含更多信息)
...
#### [插件和构建脚本的更改](#插件和构建脚本的更改)

#### [`buildSrc`设置脚本不再可见来自的类](#`buildSrc`设置脚本不再可见来自的类)
...
#### [设置脚本中加载的插件和类对项目脚本可见，并且`buildSrc`](#设置脚本中加载的插件和类对项目脚本可见，并且`buildSrc`)
...
#### [对Kotlin DSL的更改](#对Kotlin%20DSL的更改)
...
#### [Kotlin DSL IDE支持现在需要> = 1.3.50的Kotlin IntelliJ插件](#Kotlin%20DSL%20IDE支持现在需要>%20=%201.3.50的Kotlin%20IntelliJ插件)
...
#### [Kotlin DSL脚本基本类型不再扩展`Project`，`Settings`或者`Gradle`](#Kotlin%20DSL脚本基本类型不再扩展`Project`，`Settings`或者`Gradle`)
...
#### [Javadoc和Groovydoc默认不包含时间戳](#Javadoc和Groovydoc默认不包含时间戳)

生成的文档中的时间戳实际用途非常有限，但是它们使得无法构建可重复的文档。因此，`Javadoc`和`Groovydoc`任务现在已配置为默认情况下不再包括时间戳。

#### [用户提供的'config_loc'属性被Checkstyle忽略](#用户提供的'config_loc'属性被Checkstyle忽略)

`configDirectory`运行Checkstyle时，Gradle始终将其用作“ config_loc”的值。

#### [新的Tooling API进度事件](#新的Tooling%20API进度事件)
...
#### [方法打开`DefaultTask`并`ProjectLayout`替换为`ObjectFactory`](#方法打开`DefaultTask`并`ProjectLayout`替换为`ObjectFactory`)
...
#### [`ConfigurableFileCollection`任务开始执行后更改任务属性的内容](#`ConfigurableFileCollection`任务开始执行后更改任务属性的内容)
...
#### [创建`SignOperation`实例](#创建`SignOperation`实例)
...
#### [方法`WorkerExecutor.submit()`已弃用](#方法`WorkerExecutor.submit()`已弃用)
...
#### [任务依赖关系受其`@Input`值为a的任务属性的尊重`Property`](#任务依赖关系受其`@Input`值为a的任务属性的尊重`Property`)
...
#### [使用`Provider`不代表任务输出的文件声明任务依赖关系](#使用`Provider`不代表任务输出的文件声明任务依赖关系)
...
#### [设置`Property`值以`null`使用属性约定](#设置`Property`值以`null`使用属性约定)

以前，调用`Property.set(null)`总是将属性的值重置为“未定义”。现在，将使用该`convention()`方法与属性相关联的约定来确定属性的值。

#### [`publishing.publications`和的名称验证得到增强`publishing.repositories`](#`publishing.publications`和的名称验证得到增强`publishing.repositories`)

存储库名称和发布名称用于构造要发布的任务名称。可以提供一个导致无效任务名称的名称。出版物和存储库的名称现在限制为`[A-Za-z0-9_\\-.]+`。

#### [受限制的Worker API类加载器和进程类路径](#受限制的Worker%20API类加载器和进程类路径)
...
#### [默认PMD版本已升级到6.15.0](#默认PMD版本已升级到6.15.0)
...
#### [嵌入式Kotlin升级到1.3.41](#嵌入式Kotlin升级到1.3.41)
...
### [禁用JavaExec中的调试参数解析](#禁用JavaExec中的调试参数解析)
...
### [Scala 2.9和Zinc编译器](#Scala%202.9和Zinc编译器)
...
内置的[Play插件](https://docs.gradle.org/6.7.1/userguide/play_plugin.html#玩_plugin) 被弃用，将由插件门户提供的新的[Play Framework插件](https://gradle.github.io/playframework)代替。
...
#### [用户提供的Eclipse项目名称可能在冲突时被忽略](#用户提供的Eclipse项目名称可能在冲突时被忽略)
...
#### [JaCoCo的默认版本已升级到0.8.4](#JaCoCo的默认版本已升级到0.8.4)
...
#### [嵌入式Ant版本已升级到1.9.14](#嵌入式Ant版本已升级到1.9.14)

与Gradle分发的Ant版本已经升级到了[14年9月1日](https://archive.apache.org/dist/ant/RELEASE-
NOTES-1.9.14.html)从1.9.13。

#### [键入`DependencyHandler`现在静态自曝`ExtensionAware`](#键入`DependencyHandler`现在静态自曝`ExtensionAware`)
...
#### [使用自定义本地构建缓存实现](#使用自定义本地构建缓存实现)

现在不建议将自定义生成缓存实现用于本地生成缓存。唯一允许的类型将是`DirectoryBuildCache`前进。使用自定义构建缓存实现作为远程构建缓存的支持没有变化。

### [潜在的重大变化](#潜在的重大变化)

#### [通过配置Google托管库时使用HTTPS `googleApis()`](#通过配置Google托管库时使用HTTPS%20`googleApis()`)
...
#### [删除操作可以正确处理Windows上的符号链接](#删除操作可以正确处理Windows上的符号链接)
...
#### [将参数传递给Windows Resource Compiler](#将参数传递给Windows%20Resource%20Compiler)
...
#### [复制的配置不再`beforeResolve`与原始配置共享操作列表](#复制的配置不再`beforeResolve`与原始配置共享操作列表)
...
##### [行为变化以扩展任务 `AbstractArchiveTask`](#行为变化以扩展任务%20`AbstractArchiveTask`)
