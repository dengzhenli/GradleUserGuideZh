...
有关使用配置导航，检查和后处理分配的依赖项的元数据和工件的更多示例，请查看[解析结果API](https://docs.gradle.org/6.7.1/userguide/dependency_resolution.html#sec:programmatic_api)。
...
配置是Gradle中依赖关系解析的基本部分。在依赖关系解析的上下文中，区分 _消费者_ 和 _生产者_ 非常有用。按照这些原则，配置至少具有3个不同的角色：
  ...
  3. 作为 _生产者_ ，将工件及其依赖项公开以供其他项目使用（此类 _消耗性_ 配置通常表示生产者向其消费者提供的[变体](https://docs.gradle.org/6.7.1/userguide/variant_model.html)）
...
6.0开始，您需要配置[元数据源定义](https://docs.gradle.org/6.7.1/userguide/declaring_repositories.html#sec:supported_metadata_sources)以查找`hibernate-3.0.5.jar`直接调用的工件文件。  
...
，原因是它们表示文件没有附加任何[元数据](https://docs.gradle.org/6.7.1/userguide/dependency_management_terminology.html#sub:terminology_module_metadata)（例如有关传递性依赖项，来源或其作者的信息）。
...
或者，您可以还以[平面目录存储库](https://docs.gradle.org/6.7.1/userguide/declaring_repositories.html#sub:flat_dir_resolver)的形式定义一个或多个文件依赖项的源目录。
...
要将某些文件添加为配置的依赖项，只需将[文件集合](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:file_collections)作为依赖项传递：
...
#### [](#sec:versioning_file_dependencies)[文件依赖关系的版本控制](#sec:versioning_file_dependencies)

建议明确表达其意图和文件依赖性的具体版本。Gradle的[版本冲突解决方案](https://docs.gradle.org/6.7.1/userguide/dependency_resolution.html#sec:how-
...
在运行时，该构建会自动确保以正确的顺序构建项目依赖项，并将其添加到类路径中以进行编译。“[创作多项目构建](https://docs.gradle.org/6.7.1/userguide/multi_project_builds.html#multi_project_builds)”一章讨论了如何更详细地设置和配置多项目构建。
...
如果模块本身是使用Gradle构建的，则模块依赖性可以由对该模块源的本地fork的依赖性替代。这可以通过使用[复合构建](https://docs.gradle.org/6.7.1/userguide/composite_builds.html#defining_composite_builds)来完成。例如，这允许您通过使用和构建本地修补版本而不是已发布的二进制版本来解决在应用程序中使用的库中的问题。有关此内容的详细信息，请参见“[复合构建](https://docs.gradle.org/6.7.1/userguide/composite_builds.html#defining_composite_builds)”部分。

### [](#sub:gradle_distribution_dependencies)[Gradle特定于发行版的依赖性](#sub:gradle_distribution_dependencies)

#### [](#sub:api_dependencies)[Gradle
API依赖性](#sub:api_dependencies)
...
[TestKit一章](https://docs.gradle.org/6.7.1/userguide/test_kit.html#test_kit)通过示例解释了TestKit的用法。
...
声明依赖项或[依赖项约束时](https://docs.gradle.org/6.7.1/userguide/dependency_constraints.html#sec:adding-constraints-transitive-deps)，可以提供声明的自定义原因。这使构建脚本中的依赖项声明和[依赖项见解报告](https://docs.gradle.org/6.7.1/userguide/viewing_debugging_dependencies.html#sec:identifying_reason_dependency_selection)更易于解释。
...
您还可以调整将在[存储库定义中](https://docs.gradle.org/6.7.1/userguide/declaring_repositories.html#sec:supported_metadata_sources)查找哪种格式。
...
### [](#sub:supported-md-ivy)[Ivy文件](#sub:supported-md-ivy)
