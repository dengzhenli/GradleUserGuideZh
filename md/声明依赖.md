...
有关使用配置导航，检查和后处理分配的依赖项的元数据和工件的更多示例，请查看[解析结果API](/md/了解依赖性解析_md#以编程方式访问分辨率结果)。
...
6.0开始，您需要配置[元数据源定义](/md/声明存储库_md#支持的元数据源)以查找`hibernate-3.0.5.jar`直接调用的工件文件。  
...
，原因是它们表示文件没有附加任何[元数据](/md/依赖管理术语_md#模块（module）_metadata)（例如有关传递性依赖项，来源或其作者的信息）。
...
或者，您可以还以[平面目录存储库](/md/声明存储库_md#平面目录存储库)的形式定义一个或多个文件依赖项的源目录。
...
如果模块本身是使用Gradle构建的，则模块依赖性可以由对该模块源的本地fork的依赖性替代。这可以通过使用[复合构建](/md/复合构建_md#定义复合构建)来完成。例如，这允许您通过使用和构建本地修补版本而不是已发布的二进制版本来解决在应用程序中使用的库中的问题。有关此内容的详细信息，请参见“[复合构建](/md/复合构建_md#定义复合构建)”部分。

### [Gradle特定于发行版的依赖性](#Gradle特定于发行版的依赖性)

#### [Gradle%20API依赖性](#Gradle%20API依赖性)
...
#### [Gradle%20TestKit依赖性](#Gradle%20TestKit依赖性)
...
声明依赖项或[依赖项约束时](/md/升级传递依赖的版本_md#在传递依赖项上添加约束)，可以提供声明的自定义原因。这使构建脚本中的依赖项声明和[依赖项见解报告](/md/查看和调试依赖项_md#识别选择了哪个依赖版本以及为什么)更易于解释。
...
您还可以调整将在[存储库定义中](/md/声明存储库_md#支持的元数据源)查找哪种格式。
