...
带有注释的输入属性被视为Java编译类路径。除上述常规类路径规则外，编译类路径会忽略对除类文件以外的所有内容的更改。Gradle使用[Java避免编译中](https://docs.gradle.org/6.7.1/userguide/java_plugin.html#sec:java_compile_avoidance)描述的相同类分析来进一步过滤不影响类ABI的更改。这意味着仅涉及类实现的更改不会使任务过时。
[`@CompileClasspath`](https://docs.gradle.org/6.7.1/javadoc/org/gradle/api/tasks/CompileClasspath.html) [](https://docs.gradle.org/6.7.1/userguide/java_plugin.html#sec:java_compile_avoidance)
...
在[`@Nested`](https://docs.gradle.org/6.7.1/javadoc/org/gradle/api/tasks/Nested.html) 添加到地图时，然后使用键作为名称，为每个值添加一个嵌套输入。
