...
当Gradle执行任务时，它可以在控制台UI中以及通过[Tooling API](/md/Gradle和第三方工具.md#使用Tooling_API嵌入Gradle)
将任务标记为不同的结果。这些标签基于任务是否具有要执行的动作，是否应执行那些动作，是否确实执行了这些动作以及这些动作是否进行了任何更改。
...
作为示例，让我们看一下Gradle提供的`Copy`任务。要为构建创建`Copy`任务，可以在构建脚本中声明：
...
与`Task`在创建后配置可变属性相反，您可以将参数值传递给`Task`类的构造函数。为了将值传递给`Task`构造函数，您必须使用`@javax.inject.Inject`注释相关的构造函数。
