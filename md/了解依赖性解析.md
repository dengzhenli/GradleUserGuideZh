...
内容
  ...
  * [Gradle如何处理冲突？](#sec:conflict-resolution)
  * [解决版本冲突](#sec:version-conflict)
  * [解决实施冲突](#sec:implementation-conflict)
  * [Gradle如何检索依​​赖元数据？](#sec:how-gradle-downloads-deps)
  * [依赖缓存](#sec:dependency_cache)
  * [处理临时构建](#sub:ephemeral-ci-cache)
  * [以编程方式访问分辨率结果](#sec:programmatic_api)

本章介绍了Gradle _内部_
依赖解析的工作方式。在介绍了如何声明[存储库](https://docs.gradle.org/6.7.1/userguide/declaring_repositories.html)和[依赖项之后](https://docs.gradle.org/6.7.1/userguide/declaring_dependencies.html)，有必要解释在依赖项解析期间这些声明如何组合在一起。
...
## [](#sec:conflict-resolution)[Gradle如何处理冲突？](#sec:conflict-resolution)
...
依赖性解析过程可以高度自定义以满足企业需求。有关更多信息，请参见[控制传递依赖关系](https://docs.gradle.org/6.7.1/userguide/dependency_constraints.html)一章。

## [](#sec:version-conflict)[解决版本冲突](#sec:version-conflict)
...
### [](#sub:resolution-strategy)[解决策略](#sub:resolution-strategy)
...
如您所见，Gradle支持[丰富版本声明](https://docs.gradle.org/6.7.1/userguide/rich_versions.html)的概念，因此，最高版本取决于版本声明的方式：
...
请注意，在范围起作用的情况下，Gradle需要元数据来确定对于所考虑范围确实存在哪些版本。这将导致对元数据的中间查找，如[Gradle如何检索依​​赖元数据？](#sec:how-gradle-downloads-deps)。

## [](#sec:implementation-conflict)[解决实施冲突](#sec:implementation-conflict)

Gradle使用变体和功能来识别模块 _提供的功能_ 。

这是一个独特的功能，值得[一章](https://docs.gradle.org/6.7.1/userguide/variant_model.html)以了解其含义和功能。
...
在[选择候选人之间](https://docs.gradle.org/6.7.1/userguide/dependency_capability_conflict.html#sub:selecting-between-candidates)了解有关处理此类冲突的更多信息。

## [](#sec:how-gradle-downloads-deps)[Gradle如何检索依​​赖元数据？](#sec:how-gradle-downloads-deps)
...
### [](#discovering_versions)[发现版本](#discovering_versions)
...
请注意，Gradle会缓存版本信息，有关更多信息，请参见[控制动态版本缓存](https://docs.gradle.org/6.7.1/userguide/dynamic_versions.html#sec:controlling-dynamic-version-caching)一节。

### [](#obtaining_module_metadata)[获取模块元数据](#obtaining_module_metadata)
...
给定所需的依赖关系（带有版本），Gradle尝试通过搜索依赖关系指向的模块来解决依赖关系。
  ...
  * 然后，所有这些数据（包括存储库源和潜在的丢失）都存储在[Dependency Cache中](#sec:dependency_cache)。

╔═════════════════════════════  

上面的倒数第二点是使与[Maven Local](https://docs.gradle.org/6.7.1/userguide/declaring_repositories.html#sec:case-for-maven-local)集成的问题。由于它是Maven的缓存，因此有时会丢失给定模块的某些工件。如果Gradle从Maven
Local采购了这样的模块，它将认为丢失的工件完全丢失。  
  
╚═════════════════════════════    
  
### [](#repository_disabling)[仓库禁用](#repository_disabling)

当Gradle无法从存储库中检索信息时，它将在构建期间禁用它，并使所有依赖项解析失败。

最后一点对于可重复性很重要。如果允许继续构建而忽略有问题的存储库，则一旦存储库重新联机，后续的构建可能会有不同的结果。

#### [](#sub:http-retries)[HTTP重试](#sub:http-retries)

在禁用某个存储库之前，Gradle会进行几次尝试。如果连接失败，Gradle将重试某些可能会被瞬态发生的错误，从而增加每次重试之间的等待时间。

当由于永久错误或由于达到最大重试次数而无法联系存储库时，就会发生黑名单。

## [](#sec:dependency_cache)[依赖缓存](#sec:dependency_cache)
...
### [](#sub:cache_metadata)[独立的元数据缓存](#sub:cache_metadata)
...
### [](#sub:cache_repository_independence)[存储库缓存是独立的](#sub:cache_repository_independence)

如上所述，对于每个存储库，都有一个单独的元数据缓存。存储库由其URL，类型和布局标识。如果以前没有从此 _存储库_
解析模块或工件，则Gradle将尝试根据存储库解析模块。这将始终涉及对存储库的远程查找，但是在许多情况下，[不需要下载](#sub:cache_artifact_reuse)。
...
### [](#sub:cache_artifact_reuse)[工件重用](#sub:cache_artifact_reuse)
...
### [](#sub:cache_checksum_storage)[基于校验和的存储](#sub:cache_checksum_storage)
...
### [](#sub:cache_locking)[缓存锁定](#sub:cache_locking)
...
#### [](#sub:cache_cleanup)[缓存清理](#sub:cache_cleanup)

Gradle跟踪访问依赖项缓存中的哪些工件。使用此信息，定期（最多每24小时）扫描缓存，以查找未使用超过30天的工件。然后删除过时的工件，以确保高速缓存不会无限期增长。

## [](#sub:ephemeral-ci-cache)[处理临时构建](#sub:ephemeral-ci-cache)

在临时容器中运行构建是一种常见的做法。通常会生成一个容器，以便在销毁它之前仅执行一个构建。当构建依赖于每个容器必须重新下载的许多依赖项时，这可能会成为一个实际问题。为了帮助解决这种情况，Gradle提供了两个选项：

  * [将依赖项缓存复制](#sub:cache_copy)到每个容器中

  * [](#sub:shared-readonly-cache)在多个容器之间[共享只读依赖项缓存](#sub:shared-readonly-cache)

### [](#sub:cache_copy)[复制和重用缓存](#sub:cache_copy)
...
### [](#sub:shared-readonly-cache)[与其他Gradle实例共享依赖项缓存](#sub:shared-readonly-cache)

除了[将依赖项缓存复制到每个容器中之外](#sub:cache_copy)，还可以安装一个共享的只读目录，该目录将充当所有容器的依赖项缓存。与传统的依赖项高速缓存不同，此高速缓存无需锁定即可访问，从而可以从高速缓存中同时读取多个版本。重要的是，当其他构建可能正在从中读取只读缓存时，不要将其写入。
...
缓存来创建一个缓存。为此，您需要按照[上面](#sub:cache_copy)的[说明进行操作](#sub:cache_copy)。
...
## [](#sec:programmatic_api)[以编程方式访问分辨率结果](#sec:programmatic_api)
...
对于这些用例，Gradle提供了惰性的，线程安全的API，可通过调用[Configuration.getIncoming（）](https://docs.gradle.org/6.7.1/javadoc/org/gradle/api/artifacts/Configuration.html#getIncoming--)方法进行访问：
  ...
  * 的[工件视图API](https://docs.gradle.org/6.7.1/javadoc/org/gradle/api/artifacts/ResolvableDependencies.html#artifactView-org.gradle.api.Action-)提供工件，可能的一种先进的，过滤的视图[变换](https://docs.gradle.org/6.7.1/userguide/artifact_transforms.html#sec:abm_artifact_transforms)。
