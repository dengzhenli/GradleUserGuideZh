...
<h2 id = '#sec:build_phases'> <a href = '#sec:build_phases'>构建阶段</a> </h2>
...
<h2 id = '#sec:settings_file'> <a href = '#sec:settings_file'>设定档</a> </h2>
...
<h2 id = '#sec:initialization'> <a href = '#sec:initialization'>初始化</a> </h2>
...
自动搜索`settings.gradle`文件仅适用于具有默认项目布局的多项目构建，其中项目路径与磁盘上的物理子项目布局相匹配。Gradle支持多项目构建的任意物理布局，但是对于这样的任意布局，您需要从设置文件所在的目录中执行构建。有关如何从根目录运行部分构建的信息，请参阅[通过标准名称执行任务](/md/执行多项目构建.md#sec:executing_tasks_by_fully_qualified_name)。

Gradle为参与构建的每个项目创建一个Project对象。对于多项目构建，这些是Settings对象中指定的项目（加上根项目）。默认情况下，每个项目对象的名称都等于其顶级目录的名称，除根项目外的每个项目都有一个父项目。任何项目都可以有子项目。

<h2 id = '#sec:configuration_and_execution_of_a_single_project_build'> <a href = '#sec:configuration_and_execution_of_a_single_project_build'>单个项目构建的配置和执行</a> </h2>
...
<h3 id = '#sec:project_evaluation'> <a href = '#sec:project_evaluation'>项目评估</a> </h3>
...
<h3 id = '#sec:task_creation'> <a href = '#sec:task_creation'>任务创建</a> </h3>
...
<h3 id = '#sec:task_execution_graph_ready'> <a href = '#sec:task_execution_graph_ready'>任务执行图准备就绪</a> </h3>
...
<h3 id = '#sec:task_execution'> <a href = '#sec:task_execution'>任务执行</a> </h3>
