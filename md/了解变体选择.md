...
另外，为组件选择不同的工件（例如，使用`jdk7`工件）很麻烦，因为它需要使用 _分类器_。
...
在Gradle模型中，工件被附加到 _变量_ 而不是模块。实际上，这意味着不同的 _工件_
可以具有不同的依赖关系集：
...
本地组件将变体作为 _外发配置_ 公开，这是[消耗性配置](/md/声明依赖.md#可解析和消耗性配置)。
...
依赖解析引擎的作用是给定一个表示的约束 _消费者_ ，寻找合适的 _变种_ 的 _生产商_ 。
...
这意味着外部组件提供了 _variants_ ，它们也具有属性。但是，有时由于历史原因或由于您使用也具有这种 _配置_ 概念的Ivy，
术语 _配置_ 可能会泄漏到DSL中。   
...
Gradle提供了一个名为Report的报告任务`outgoingVariants`，该任务显示项目的变体及其功能，属性和工件。
...
它还显示了 _次级_变体，这些变体是Gradle项目专有的，未发布。
例如，`classes`from的第二个变体`apiElements`是让Gradle在针对[`java-library`project进行](/md/Java库插件.md#使用类而不是jar进行编译)编译时跳过JAR创建。
...
组件可以公开的变体数量没有限制。传统上，一个组件会公开一个API和一个实现，但是例如，我们可能也想公开一个组件的测试装置。
