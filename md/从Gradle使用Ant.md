...
您可以使用Gradle的依赖关系管理来组装用于自定义任务的类路径。为此，您需要为类路径定义一个自定义配置，然后向该配置中添加一些依赖项。这在[声明依赖项](https://docs.gradle.org/6.7.1/userguide/declaring_dependencies.html)中有更详细的描述。
...
## [](#sec:ant_properties)[Ant属性和参考](#sec:ant_properties)
...
## [](#sec:ant_logging)[Ant记录](#sec:ant_logging)
