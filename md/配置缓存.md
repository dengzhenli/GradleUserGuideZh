...
## [Adoption%20steps%20（采纳步骤)](#Adoption%20steps%20（采纳步骤))
...
否则，您可以使用[注入服务](/md/开发自定义Gradle类型_md#服务注入)代替的方法`Project`。
...
任务不应直接访问另一个任务实例的状态。相反，应该使用[输入和输出关系](/md/延迟配置_md#使用任务输入和输出)来连接任务。
...
使用配置缓存时，不会捕获自定义[输入归一化](/md/处理任务_md#配置输入标准化)。如果您的构建具有用于输入归一化的配置，即`normalization
