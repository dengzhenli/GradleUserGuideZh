...
metadata`标志要求您要生成或用于[签名](#验证依赖项签名)的[校验和](#验证依赖项校验和)的列表。`pgp`[](#验证依赖项签名)

执行此命令行将使Gradle执行以下操作：

  * 解决所有[可解决的配置](/md/声明依赖_md#可解析和消耗性配置)，包括：
...
清理文件的一种方法是将现有`verification-metadata.xml`文件移动到其他位置，并使用以下[`--dry-run`方式](#使用干燥模式)调用Gradle ：虽然不完善（它不会注意到依赖项仅在配置时解决），但它会生成 _一个新文件_ ，您可以将其与现有的。
