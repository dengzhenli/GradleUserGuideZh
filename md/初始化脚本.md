...
初始化脚本的一个主要限制是它们不能访问`buildSrc`项目中的类（有关此功能的详细信息，请参见[使用buildSrc提取命令式逻辑](/md/组织Gradle项目_md#用`buildSrc`抽象逻辑势在必行)）。
...
在[构建脚本的外部依赖关系中，](/md/构建脚本基础_md#构建脚本的外部依赖关系)说明了如何向构建脚本添加外部依赖关系。初始化脚本也可以声明依赖关系。您可以使用`initscript()`方法执行此操作，并传入一个声明初始化脚本类路径的闭包。
