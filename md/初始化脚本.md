...
内容
  ...
  * [基本用法](#sec_basic_usage)
  * [使用初始化脚本](#sec_using_an_init_script)
  * [编写一个初始化脚本](#sec_writing_an_init_script)
  * [初始化脚本的外部依赖关系](#sec_custom_classpath)
  * [初始化脚本插件](#sec_init_script_plugins)
...
<h2 id = '#sec_basic_usage'> <a href = '#sec_basic_usage'>基本用法</a> </h2>
...
初始化脚本的一个主要限制是它们不能访问`buildSrc`项目中的类（有关此功能的详细信息，请参见[使用buildSrc提取命令式逻辑](/md/组织Gradle项目.md#sec_build_sources)）。

<h2 id = '#sec_using_an_init_script'> <a href = '#sec_using_an_init_script'>使用初始化脚本</a> </h2>
...
<h2 id = '#sec_writing_an_init_script'> <a href = '#sec_writing_an_init_script'>编写一个初始化脚本</a> </h2>
...
<h3 id = '#sec_configuring_projects_from_an_init_script'> <a href = '#sec_configuring_projects_from_an_init_script'>通过初始化脚本配置项目</a> </h3>
...
<h4 id = '#output_when_applying_the_init_script'> <a href = '#output_when_applying_the_init_script'>应用初始化脚本时的输出</a> </h4>
...
<h2 id = '#sec_custom_classpath'> <a href = '#sec_custom_classpath'>初始化脚本的外部依赖关系</a> </h2>

在[构建脚本的外部依赖关系中，](/md/构建脚本基础.md#sec_build_script_external_dependencies)说明了如何向构建脚本添加外部依赖关系。初始化脚本也可以声明依赖关系。您可以使用`initscript()`方法执行此操作，并传入一个声明初始化脚本类路径的闭包。
...
<h2 id = '#sec_init_script_plugins'> <a href = '#sec_init_script_plugins'>初始化脚本插件</a> </h2>
...
在初始化脚本中应用插件时，Gradle实例化插件并调用插件实例的[Plugin.apply（T）](https://docs.gradle.org/6.7.1/javadoc/org/gradle/api/Plugin.html#apply-T-)方法。该`gradle`对象作为参数传递，可用于配置构建的各个方面。当然，可以将应用的插件解析为外部依赖项，如[init脚本的外部依赖项中所述](#sec_custom_classpath)
