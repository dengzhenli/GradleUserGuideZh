...
内容
  ...
  * [实现工件转换](#sec_implementing-artifact-transforms)
  * [注册工件转换](#registering_artifact_transforms)
  * [实现增量工件转换](#implementing_incremental_artifact_transforms)

如在[不同类型的配置中所述](/md/声明依赖.md#sec_resolvable-consumable-configs)，对于相同的依赖项可能会有不同的变体。例如，外部Maven依赖项具有针对依赖项（`java-
...
Gradle通过其[属性](/md/使用变体属性.md#sec_abm_configuration_attributes)集唯一地标识依赖项的变体。的`java-
api`依赖关系的变体是由所识别的所述变体`org.gradle.usage`与值属性`java-api`。

当Gradle解析配置时，解析的配置上的[属性](/md/使用变体属性.md#sec_abm_configuration_attributes)将确定
...
<h2 id = '#artifact_transform_selection_and_execution'> <a href = '#artifact_transform_selection_and_execution'>工件转换的选择和执行</a> </h2>
...
<h2 id = '#sec_implementing-artifact-transforms'> <a href = '#sec_implementing-artifact-transforms'>实现工件转换</a> </h2>
...
⑥  使用参数  
╚═════════════════════════════    
<h2 id = '#registering_artifact_transforms'> <a href = '#registering_artifact_transforms'>注册工件转换</a> </h2>
...
<h2 id = '#implementing_incremental_artifact_transforms'> <a href = '#implementing_incremental_artifact_transforms'>实现增量工件转换</a> </h2>

与[增量任务](/md/开发自定义Gradle任务类型.md#incremental_tasks)类似，工件转换可以通过仅处理上次执行后更改的文件来避免工作。这是通过使用[InputChanges](https://docs.gradle.org/6.7.1/dsl/org.gradle.work.InputChanges.html)接口完成的。对于工件转换，只有输入工件是增量输入，因此转换只能查询那里的更改。为了在转换动作中使用[InputChanges](https://docs.gradle.org/6.7.1/dsl/org.gradle.work.InputChanges.html)，请将其注入到动作中。有关如何使用[InputChanges的](https://docs.gradle.org/6.7.1/dsl/org.gradle.work.InputChanges.html)更多信息，请参见有关[增量任务](/md/开发自定义Gradle任务类型.md#incremental_tasks)的相应文档。
