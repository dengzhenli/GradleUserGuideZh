...
如在[不同类型的配置中所述](/md/声明依赖_md#可解析和消耗性配置)，对于相同的依赖项可能会有不同的变体。例如，外部Maven依赖项具有针对依赖项（`java-
