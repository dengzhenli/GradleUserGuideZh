...
如在[不同类型的配置中所述](/md/声明依赖.md#sec:resolvable-consumable-configs)，对于相同的依赖项可能会有不同的变体。例如，外部Maven依赖项具有针对依赖项（`java-
...
<h2 id = '#sec:implementing-artifact-transforms'> <a href = '#sec:implementing-artifact-transforms'>实现工件转换</a> </h2>
