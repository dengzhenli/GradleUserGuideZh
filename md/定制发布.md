...
。例如，如果您[为其他平台添加了Java库的变体](https://docs.gradle.org/6.7.1/userguide/cross_project_publications.html#targeting-different-platforms)，则可能只想在`java`组件本身上声明此附加变体。通常，声明其他变体通常是发布 _其他工件_ 的最佳解决方案。

为了执行此类添加或修改，`AdhocComponentWithVariants`接口声明了两个名为的方法`addVariantsFromConfiguration`，`withVariantsFromConfiguration`它们接受两个参数：

  * 用作变体源的[传出配置](https://docs.gradle.org/6.7.1/userguide/declaring_dependencies.html#sec:resolvable-consumable-configs)
...
API。对于自定义组件，第一步是按照[本章中](https://docs.gradle.org/6.7.1/userguide/cross_project_publications.html#sec:variant-aware-sharing)的说明创建自定义传出变型。在此阶段，您应该拥有可以在跨项目依赖项中使用的变体，但现在我们将其发布到外部存储库。
...
_次要变体_
）相关联。这样的配置在[跨项目发布](https://docs.gradle.org/6.7.1/userguide/cross_project_publications.html)用例中有意义，但在外部发布时则没有意义。例如，在项目之间共享
...
与其考虑工件，不如考虑Gradle的变体感知模型。预期单个模块可能需要多个工件。但是，这种情况很少在这里停下来，如果其他工件代表[可选功能](https://docs.gradle.org/6.7.1/userguide/feature_variants.html)，它们可能也具有不同的依赖关系以及更多依赖关系。

Gradle通过 _Gradle Module Metadata_ 支持 _其他变体_ 的发布，这些 _变体_
使依赖解析引擎知道这些工件。请参阅文档的[变量感知共享](https://docs.gradle.org/6.7.1/userguide/cross_project_publications.html#sec:variant-aware-sharing)部分，以了解如何声明此类变量并[了解如何发布自定义组件](#sec:publishing-custom-components)。
...
要基于工件创建发布，请先定义一个自定义工件并将其附加到您选择的Gradle[配置](https://docs.gradle.org/6.7.1/userguide/dependency_management_terminology.html#sub:terminology_configuration)。以下示例定义了由`rpm`任务（未显示）产生的RPM工件，并将该工件附加到`archives`配置：
...
build.gradle.kts
  ...
  * 发布插件支持不同的工件配置属性，因此请始终查看插件文档以获取更多详细信息。将`classifier`和`extension`属性由两个支持[Maven的发布插件](https://docs.gradle.org/6.7.1/userguide/publishing_maven.html#publishing_maven)和[Ivy发布插件](https://docs.gradle.org/6.7.1/userguide/publishing_ivy.html#publishing_ivy)。
