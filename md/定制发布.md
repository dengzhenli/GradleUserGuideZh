...
。例如，如果您[为其他平台添加了Java库的变体](/md/在项目之间共享输出.md#针对不同的平台)，则可能只想在`java`组件本身上声明此附加变体。通常，声明其他变体通常是发布 _其他工件_ 的最佳解决方案。
...
API。对于自定义组件，第一步是按照[本章中](/md/在项目之间共享输出.md#项目之间工件的变体感知共享)的说明创建自定义传出变型。在此阶段，您应该拥有可以在跨项目依赖项中使用的变体，但现在我们将其发布到外部存储库。
...
使依赖解析引擎知道这些工件。请参阅文档的[变量感知共享](/md/在项目之间共享输出.md#项目之间工件的变体感知共享)部分，以了解如何声明此类变量并[了解如何发布自定义组件](#%E5%88%9B%E5%BB%BA%E5%92%8C%E5%8F%91%E5%B8%83%E8%87%AA%E5%AE%9A%E4%B9%89%E7%BB%84%E4%BB%B6)。
