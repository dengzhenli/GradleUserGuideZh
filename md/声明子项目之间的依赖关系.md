...
如果一个项目需要另一个项目在其编译类路径上生成的jar，该怎么办？如果它还需要另一个项目的传递依赖关系，该怎么办？显然，这是Java多项目构建的非常常见的用例。如[项目依赖项中所述](/md/声明依赖.md#sub:project_dependencies)，Gradle为此提供了项目依赖项。
...
仅依赖于另一个项目的主要输出工件。Gradle具有最强大的[依赖性管理引擎之一](/md/Gradle中的依赖管理.md#sec:dependency-mgmt-in-gradle)，可让您在项目之间共享任意工件，并让Gradle按需构建它们。有关更多详细信息，请参见有关[在项目之间共享输出](/md/在项目之间共享输出.md#cross_project_publications)的部分。
