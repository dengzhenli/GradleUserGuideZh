...
内容
  ...
  * [用法](#用法)
  * [API与实现分离](#API与实现分离)
  * [识别API和实现依赖性](#识别API和实现依赖性)
  * [Java库插件配置](#Java库插件配置)
  * [Java模块系统的构建模块](#Java模块系统的构建模块)
  * [使用类而不是jar进行编译](#使用类而不是jar进行编译)
  * [分发库](#分发库)
...
的替代方法，它是告诉编译器有关已编译依赖项的传统方式。如果满足以下三个条件，则Gradle会自动将您的依赖项Jar放在模块路径上，而不是在类路径上：
  ...
  * 实际上，我们正在构建一个模块（与传统库相反），通过添加`module-info.java`文件来表示该模块。（另一种选择是添加`Automatic-Module-Name`Jar清单属性，[如](#建立一个自动模块)后面[所述](#建立一个自动模块)。）
...
_buildSrc_
插件，该插件注册了您可以使用并根据需要进行调整的转换。如果您想构建一个完全[模块化的应用程序](/md/Java应用插件_md#使用Java模块系统构建应用程序)并希望Java运行时将所有内容都视为一个真正的模块，那么这可能会很有趣。
