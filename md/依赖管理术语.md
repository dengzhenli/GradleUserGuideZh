...
内容
  ...
  * [功能变体（feature variant）](#功能变体（feature%20variant）)
  * [模块（module）](#模块（module）)
  * [模块元数据（module metadata）](#模块元数据（module%20metadata）)
  * [组件元数据规则](#组件元数据规则)
  * [模块版本（module version）](#模块版本（module%20version）)
...
功能标识由一个或多个组件提供的功能。通过类似于用于[模块版本（module version）](#模块版本（module%20version）)的坐标的坐标来标识capability。默认情况下，每个模块版本（module version）均提供与其坐标匹配的功能，例如`com.google:guava:18.0`。功能可以用来表示一个组件提供了多个[功能变体（feature variant），](#功能变体（feature%20variant）)或者两个不同的组件实现了相同的功能（因此不能一起使用）。有关更多详细信息，请参见[功能](/md/声明Library的能力.md)部分。
...
请参见有关[依赖项配置](/md/声明依赖_md#什么是依赖项配置)以及[可解析和易用配置的各节](/md/声明依赖_md#可解析和消耗性配置)。
...
## [依赖约束（dependency%20constraint）](#依赖（dependency）_constraint)
...
## [功能变体（feature%20variant）](#功能变体（feature%20variant）)
...
随时间推移而发展的软件，例如[Google Guava](https://github.com/google/guava)。每个模块（module）都有一个名称。模块（module）的每个发行版均以[模块版本（module version）](#模块版本（module%20version）)为代表。为了方便使用，可以将模块（module）托管在[资源库中](#repository)。

## [模块元数据（module%20metadata）](#模块元数据（module%20metadata）)
...
## [模块版本（module%20version）](#模块版本（module%20version）)
...
另外，Gradle定义了[虚拟平台（platform）](/md/对齐依赖版本_md#在没有发布平台的情况下对齐模块版本)。

╔═════════════════════════════  

Maven的BOM（物料清单）是[Gradle支持的](/md/在项目之间共享依赖版本_md#导入Maven%20BOM)一种流行的平台（platform）。  
...
作品（publication）具有名称，由一个或多个工件以及有关这些工件的信息（[元数据](#模块元数据（module%20metadata）)）组成。

## [repository](#repository)

存储库托管一组[模块（module）](#模块（module）)，每个[模块（module）](#模块（module）)可以提供一个或多个由[模块版本（module version）](#模块版本（module%20version）)指示的发行版（组件）。该存储库可以基于二进制存储库产品（例如Artifactory或Nexus）或文件系统中的目录结构。有关更多信息，请参见[声明存储库](/md/声明存储库.md)。
...
[组件](#component)的变体可以依赖于其他模块（module）才能正常工作，即所谓的传递依赖。托管在[存储库](#repository)中的模块（module）的发行版可以提供[元数据](#模块元数据（module%20metadata）)来声明这些可传递依赖项。默认情况下，Gradle自动解析传递依赖。声明[依赖约束（dependency constraint））](/md/升级传递依赖的版本.md)可以影响可传递依赖的版本选择。
