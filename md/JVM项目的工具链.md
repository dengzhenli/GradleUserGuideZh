...
内容
  ...
  * [自动检测已安装的工具链](#sec:auto_detection)
  * [自动配置](#sec:provisioning)
  * [自定义工具链位置](#sec:custom_loc)
  * [插件作者的工具链](#sec:plugins)
...
现在，执行构建（例如使用`gradle check`）将为您和其他运行构建的人处理几件事
  ...
  2. Gradle检测[本地安装的JVM](#sec:auto_detection)
...
JDK。预配JDK安装在[Gradle用户主目录中](https://docs.gradle.org/6.7.1/userguide/directory_layout.html#dir:gradle_user_home)。
