...
内容
  ...
  * [日志级别](#logLevels)
  * [选择日志级别](#sec_choosing_a_log_level)
  * [记录敏感信息](#sec_debug_security)
  * [编写自己的日志消息](#sec_sending_your_own_log_messages)
  * [从外部工具和库记录](#sec_external_tools)
  * [更改Gradle日志](#sec_changing_what_gradle_logs)
...
<h2 id = '#logLevels'> <a href = '#logLevels'>日志级别</a> </h2>
日志级别|说明
---|---  
ERROR  |错误讯息  
QUIET  |重要信息消息  
...
<h2 id = '#sec_choosing_a_log_level'> <a href = '#sec_choosing_a_log_level'>选择日志级别</a> </h2>

您可以使用“[日志级别”命令行选项中](#logLevelCommandLineOptions)显示的命令行开关来选择不同的日志级别。您还可以使用来配置日志级别`gradle.properties`，请参阅[Gradle属性](/md/Gradle环境搭建.md#sec_gradle_configuration_properties)。在[Stacktrace命令行选项中，](#stacktraces)您会找到影响stacktrace日志记录的命令行开关。
...
该`DEBUG`日志级别可以[暴露的安全敏感信息到控制台](#sec_debug_security)。  
  
╚═════════════════════════════  
  
<h3 id = '#stacktraces'> <a href = '#stacktraces'>Stacktrace命令行选项</a> </h3>
...
<h2 id = '#sec_debug_security'> <a href = '#sec_debug_security'>记录敏感信息</a> </h2>
...
<h2 id = '#sec_sending_your_own_log_messages'> <a href = '#sec_sending_your_own_log_messages'>编写自己的日志消息</a> </h2>
...
<h2 id = '#sec_external_tools'> <a href = '#sec_external_tools'>从外部工具和库记录</a> </h2>
...
<h2 id = '#sec_changing_what_gradle_logs'> <a href = '#sec_changing_what_gradle_logs'>更改Gradle日志</a> </h2>
...
您的记录器可以实现下面列出的任何侦听器接口。注册记录器时，只会替换其实现的接口的记录。其他接口的日志保持不变。您可以在“[构建生命周期”事件中](/md/构建生命周期.md#build_lifecycle_events)找到有关侦听器接口的更多信息。
