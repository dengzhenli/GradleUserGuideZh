...
您可以使用“[日志级别”命令行选项中](#logLevelCommandLineOptions)显示的命令行开关来选择不同的日志级别。您还可以使用来配置日志级别`gradle.properties`，请参阅[Gradle属性](/md/Gradle环境搭建_md#Gradle属性)。在[Stacktrace命令行选项中，](#Stacktrace命令行选项)您会找到影响stacktrace日志记录的命令行开关。
...
您的记录器可以实现下面列出的任何侦听器接口。注册记录器时，只会替换其实现的接口的记录。其他接口的日志保持不变。您可以在“[构建生命周期”事件中](/md/构建生命周期_md#响应构建脚本中的生命周期)找到有关侦听器接口的更多信息。
