...
内容
  ...
  * [避免使用内部Gradle API](#避免使用内部Gradle%20API)
  * [声明任务时遵循约定](#声明任务时遵循约定)
  * [提高任务发现能力](#提高任务发现能力)
...
让我们将构建脚本与实现为二进制插件的相同逻辑进行比较。该代码起初看起来可能涉及更多，但显然看起来更像典型的应用程序代码。这个特定的插件类位于[`buildSrc`目录中](/md/组织Gradle项目_md#用`buildSrc`抽象逻辑势在必行)，该[目录](/md/组织Gradle项目_md#用`buildSrc`抽象逻辑势在必行)可自动用于构建脚本。
...
## [避免使用内部Gradle%20API](#避免使用内部Gradle%20API)
...
Gradle并不限制构建脚本作者从一个项目进入[多](/md/Gradle中的多项目构建.md#multi_project_builds)模型构建中的另一个项目的领域模型。高度耦合的项目会损害[构建执行性能](/md/配置时间和执行时间_md#并行项目执行)以及代码的可读性和可维护性。
...
以及[Artifact存储库](/md/声明存储库_md#处理凭证)
，该API允许
在构建需要它们时使用[Gradle属性](/md/Gradle环境搭建_md#Gradle属性)提供凭证值。这样，凭据可以存储在`gradle.properties`用户主目录中的文件中，也可以使用命令行参数或环境变量注入到构建中。
