...
，该API允许
在构建需要它们时使用[Gradle属性](/md/Gradle环境搭建.md#Gradle属性)提供凭证值。这样，凭据可以存储在`gradle.properties`用户主目录中的文件中，也可以使用命令行参数或环境变量注入到构建中。
