...
内容
  ...
  * [避免在脚本中使用命令式逻辑](#sec_avoid_imperative_logic_in_scripts)
  * [避免使用内部Gradle API](#sec_avoiding_gradle_internal_apis)
  * [声明任务时遵循约定](#sec_declaring_tasks)
  * [提高任务发现能力](#sec_improving_task_discoverability)
  * [最小化在配置阶段执行的逻辑](#sec_minimize_logic_executed_configuration_phase)
  * [避免使用`GradleBuild`任务类型](#sec_avoiding_use_of_gradlebuild)
  * [避免项目间配置](#sec_avoiding_inter_project_configuration)
  * [外部化和加密您的密码](#sec_avoiding_passwords_in_plain_text)
...
<h2 id = '#sec_avoid_imperative_logic_in_scripts'> <a href = '#sec_avoid_imperative_logic_in_scripts'>避免在脚本中使用命令式逻辑</a> </h2>
...
让我们将构建脚本与实现为二进制插件的相同逻辑进行比较。该代码起初看起来可能涉及更多，但显然看起来更像典型的应用程序代码。这个特定的插件类位于[`buildSrc`目录中](/md/组织Gradle项目.md#sec_build_sources)，该[目录](/md/组织Gradle项目.md#sec_build_sources)可自动用于构建脚本。
...
<h2 id = '#sec_avoiding_gradle_internal_apis'> <a href = '#sec_avoiding_gradle_internal_apis'>避免使用内部Gradle API</a> </h2>
...
<h3 id = '#alternatives_for_oft_used_internal_apis'> <a href = '#alternatives_for_oft_used_internal_apis'>常用内部API的替代方案</a> </h3>
...
<h2 id = '#sec_declaring_tasks'> <a href = '#sec_declaring_tasks'>声明任务时遵循约定</a> </h2>
...
任务API为构建作者提供了很大的灵活性，使其可以在构建脚本中声明任务。为了获得最佳的可读性和可维护性，请遵循以下规则：
  ...
  * 一个任务应该[定义一个组和描述](#sec_improving_task_discoverability)。
...
<h2 id = '#sec_improving_task_discoverability'> <a href = '#sec_improving_task_discoverability'>提高任务发现能力</a> </h2>

即使是构建的新用户，也应该能够快速而轻松地找到关键信息。在Gradle中，您可以为构建的任何任务声明一个[组](https://docs.gradle.org/6.7.1/dsl/org.gradle.api.Project.html#org.gradle.api.Project:group)和一个[描述](https://docs.gradle.org/6.7.1/dsl/org.gradle.api.Project.html#org.gradle.api.Project:description)。在[任务报告](/md/命令行界面.md#sec_listing_tasks)使用指定的值来组织和呈现，便于发现性的任务。分配组和描述对于您希望构建用户调用的任何任务最有帮助。
...
<h2 id = '#sec_minimize_logic_executed_configuration_phase'> <a href = '#sec_minimize_logic_executed_configuration_phase'>最小化在配置阶段执行的逻辑</a> </h2>
...
<h2 id = '#sec_avoiding_use_of_gradlebuild'> <a href = '#sec_avoiding_use_of_gradlebuild'>避免使用`GradleBuild`任务类型</a> </h2>
...
<h2 id = '#sec_avoiding_inter_project_configuration'> <a href = '#sec_avoiding_inter_project_configuration'>避免项目间配置</a> </h2>

Gradle并不限制构建脚本作者从一个项目进入[多](/md/Gradle中的多项目构建.md#multi_project_builds)模型构建中的另一个项目的领域模型。高度耦合的项目会损害[构建执行性能](/md/配置时间和执行时间.md#sec_parallel_execution)以及代码的可读性和可维护性。
...
应避免以下做法：
  ...
  * 用[GradleBuild](#sec_avoiding_use_of_gradlebuild)执行构建的另一部分。

  * 声明不必要的[项目依赖项](/md/声明依赖.md#sub_project_dependencies)。

<h2 id = '#sec_avoiding_passwords_in_plain_text'> <a href = '#sec_avoiding_passwords_in_plain_text'>外部化和加密您的密码</a> </h2>
...
以及[Artifact存储库](/md/声明存储库.md#sec_handling_credentials)
，该API允许
在构建需要它们时使用[Gradle属性](/md/Gradle环境搭建.md#sec_gradle_configuration_properties)提供凭证值。这样，凭据可以存储在`gradle.properties`用户主目录中的文件中，也可以使用命令行参数或环境变量注入到构建中。
