...
该[GradleBuild](https://docs.gradle.org/6.7.1/dsl/org.gradle.api.tasks.GradleBuild.html)任务类型允许构建脚本定义一个任务调用另一个摇篮构建。通常不建议使用此类型。在某些极端情况下，所调用的构建不会像从命令行或通过Tooling
