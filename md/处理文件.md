...
内容
  ...
  * [重命名复制中的文件](#sec:renaming_files_example)
...
Plugin](https://docs.gradle.org/6.7.1/userguide/base_plugin.html#base_plugin)。它为这些属性提供了一些常规值。下一个示例对此进行了演示，您可以在[归档命名](#sec:archive_naming)部分中了解有关约定的更多信息。
...
## [](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:unpacking_archives_example)[解压缩档案](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:unpacking_archives_example)

档案实际上是独立的文件系统，因此解压缩它们是将文件从该文件系统复制到本地文件系统，甚至复制到另一个档案的情况。\
Gradle通过提供一些包装器功能来实现此目的，这些包装器功能使档案可以作为文件（[文件树](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:file_trees)）的分层集合使用。
...
与普通副本一样，您可以控制通过[过滤器](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:filtering_files)解压缩哪些文件，甚至可以在解压缩文件时[重命名文件](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:renaming_files)。
...
## [](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:creating_uber_jar_example)[创建“超级”或“胖”
JAR](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:creating_uber_jar_example)
...
## [](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:creating_directories_example)[创建目录](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:creating_directories_example)

许多任务需要创建目录来存储它们生成的文件，这就是为什么Gradle在明确定义文件和目录输出时会自动管理任务的这一方面的原因。您可以在用户手册的[增量构建](https://docs.gradle.org/6.7.1/userguide/more_about_tasks.html#sec:up_to_date_checks)部分中了解此功能。所有Gradle核心任务都确保必要时使用此机制创建所需的任何输出目录。
...
## [](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:moving_files_example)[移动文件和目录](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:moving_files_example)

Gradle没有用于移动文件和目录的API，但是您可以使用[Apache
Ant集成](https://docs.gradle.org/6.7.1/userguide/ant.html#ant)轻松地做到这一点，如以下示例所示：
...
## [](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:renaming_files_example)[重命名复制中的文件](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:renaming_files_example)
...
## [](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:deleting_files_example)[删除文件和目录](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:deleting_files_example)
...
## [](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:locating_files)[深度文件路径](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:locating_files)
...
### [](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:hard_coded_file_paths)[在硬编码的文件路径上](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:hard_coded_file_paths)

本章中的许多示例都将硬编码路径用作字符串文字。这使它们易于理解，但是对于实际构建而言，这不是一个好习惯。问题在于路径经常更改，而您需要更改的位置越多，您越有可能错过其中一个并破坏构建。

在可能的情况下，应使用任务，任务属性和[项目属性](https://docs.gradle.org/6.7.1/userguide/writing_build_scripts.html#sec:extra_properties)（按优先顺序）来配置文件路径。例如，如果要创建一个打包Java应用程序的已编译类的任务，则应针对以下目标：
...
直接将任务用作这样的参数依赖于它具有已[定义的output](https://docs.gradle.org/6.7.1/userguide/more_about_tasks.html#sec:task_inputs_outputs)，因此并非总是可能的。另外，可以通过依靠Java插件的约定`destinationDirectory`而不是覆盖该约定来进一步改进此示例，但是它确实演示了项目属性的使用。

### [](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:single_file_paths)[单个文件和目录](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:single_file_paths)
...
### [](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:file_collections)[文件收集](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:file_collections)
...
API的许多部分都在使用`FileCollection`，例如本章稍后讨论的复制API和[依赖项配置](https://docs.gradle.org/6.7.1/userguide/declaring_dependencies.html#sec:what-
...
java.lang.Object...-)方法，该方法返回一个`FileCollection`实例。此方法非常灵活，可让您传递多个字符串，`File`实例，字符串集合，`File`s集合等。如果任务[定义了输出，](https://docs.gradle.org/6.7.1/userguide/more_about_tasks.html#sec:task_inputs_outputs)您甚至可以将任务作为参数传递。在参考指南中了解所有受支持的参数类型。
...
与[上一节中](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:single_file_paths)介绍的[Project.file（java.lang.Object）](https://docs.gradle.org/6.7.1/dsl/org.gradle.api.Project.html#org.gradle.api.Project:file\(java.lang.Object\))方法一样，所有相对路径都相对于当前项目目录进行求值。以下示例演示了您可以使用的各种参数类型-
字符串，实例，列表和a
：[](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:single_file_paths)`File``[Path](https://docs.oracle.com/javase/8/docs/api/java/nio/file/Path.html)`
...
### [](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:file_trees)[文件树](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:file_trees)
...
我们已经讨论了如何创建自己的文件树和文件集合，但是也要记住，许多Gradle插件提供了自己的文件树实例，例如[Java的源集](https://docs.gradle.org/6.7.1/userguide/building_java_projects.html#sec:java_source_sets)。这些文件的使用和操作方式与您自己创建的文件树完全相同。

用户通常需要的另一种特定类型的文件树是归档文件，即ZIP文件，TAR文件等。我们接下来将介绍它们。

### [](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:archive_contents)[将存档用作文件树](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:archive_contents)
...
[在](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:unpacking_archives_example)我们介绍[的常见方案中，](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:unpacking_archives_example)您可以看到提取存档文件的实际示例。

### [](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:specifying_multiple_files)[了解对文件集合的隐式转换](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:specifying_multiple_files)
...
docs中提到的[files（）](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:file_collections)方法支持的任何类型来设置此属性的值。这意味着你可以，例如，属性设置为`File`，`String`，收藏，`FileCollection`甚至关闭或`Provider`。
...
到值的集合，而不是替换它们。同样，此方法接受[files（）](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:file_collections)方法支持的任何类型，如下所示：
...
## [](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:copying_files)[深度复制文件](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:copying_files)
...
不仅`from()`接受多个参数，还允许几种不同类型的参数。例如，一些最常见的类型是：
  ...
  * 任务-包含构成任务[定义的输出](https://docs.gradle.org/6.7.1/userguide/more_about_tasks.html#sec:task_inputs_outputs)的文件或目录
...
请注意，即使语法相似，的惰性配置`into()`也不同于[子规范](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sub:using_child_copy_specifications)。注意参数的数量以区分它们。

### [](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#filtering_files)[过滤文件](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#filtering_files)
...
`from()`配置。如果要将过滤应用于复制文件的子集，则需要使用[子规范](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sub:using_child_copy_specifications)。

### [](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:renaming_files)[重命名文件](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:renaming_files)

[如何重命名复制文件](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:renaming_files_example)的[示例](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:renaming_files_example)为您提供了执行此操作所需的大多数信息。它演示了重命名的两个选项：
...
### [](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:filtering_files)[过滤文件内容（令牌替换，模板等）](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:filtering_files)
...
### [](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:using_the_copyspec_class)[使用`CopySpec`课程](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:using_the_copyspec_class)
...
#### [](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sub:sharing_copy_specs)[共享复印规格](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sub:sharing_copy_specs)
...
#### [](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sub:using_child_copy_specifications)[使用子规范](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sub:using_child_copy_specifications)
...
### [](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:project_copy_method)[在自己的任务中复制文件](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:project_copy_method)
...
上面的示例演示了基本语法，还强调了使用该`copy()`方法的两个主要限制：
  ...
  1. 该`copy()`方法不是[增量的](https://docs.gradle.org/6.7.1/userguide/more_about_tasks.html#sec:up_to_date_checks)。该示例的`copyMethod`任务将 _始终_ 执行，因为它没有有关组成任务输入的文件的信息。您必须手动定义任务输入和输出。

  2. 将任务用作复制源，即作为的参数`from()`，不会在您的任务和该复制源之间建立自动的任务依赖关系。因此，如果将`copy()`方法用作任务操作的一部分，则必须显式声明所有输入和输出，以获得正确的行为。

以下示例显示了如何通过使用[动态API进行任务输入和输出](https://docs.gradle.org/6.7.1/userguide/more_about_tasks.html#sec:task_input_output_runtime_api)来解决这些限制：
...
这些限制使得最好`Copy`在可能的情况下使用任务，因为它内置了对增量构建和任务依赖性推断的支持。这就是为什么该`copy()`方法供需要复制文件作为其功能一部分的[自定义任务](https://docs.gradle.org/6.7.1/userguide/custom_tasks.html#custom_tasks)使用的原因。使用该`copy()`方法的自定义任务应声明与复制操作有关的必要输入和输出。

### [](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:sync_task)[使用`Sync`任务镜像目录和文件集合](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:sync_task)
...
## [](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:archives)[深入创建档案](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:archives)
...
[子规范](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sub:using_child_copy_specifications)来实现。
...
### [](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:archive_naming)[档案命名](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:archive_naming)
...
### [](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:sharing_content_between_multiple_archives)[在多个档案之间共享内容](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:sharing_content_between_multiple_archives)

[如前所述](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sub:sharing_copy_specs)，您可以使用[Project.copySpec（org.gradle.api.Action）](https://docs.gradle.org/6.7.1/dsl/org.gradle.api.Project.html#org.gradle.api.Project:copySpec\(org.gradle.api.Action\))方法在存档之间共享内容。

### [](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:reproducible_archives)[可复制的构建](https://docs.gradle.org/6.7.1/userguide/working_with_files.html#sec:reproducible_archives)
