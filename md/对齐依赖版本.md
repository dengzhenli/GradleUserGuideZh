...
Gradle本机支持Gradle生产的模块的对齐。这是[依赖性约束](/md/升级传递依赖的版本_md#在传递依赖项上添加约束)
的[可传递性](/md/升级传递依赖的版本_md#在传递依赖项上添加约束)的直接结果。因此，如果您具有多项目构建，并且希望使用者获得所有模块的相同版本，那么Gradle提供了一种使用
...
每当发布者不使用Gradle时（例如在我们的Jackson例子中），我们都可以向Gradle解释说，所有Jackson模块都“属于”同一平台，并从与本地对齐相同的行为中受益。有两个选项可以表示一组模块属于某个平台：
  ...
  1. 平台作为[BOM](/md/在项目之间共享依赖版本_md#导入Maven%20BOM) **发布** ，可以使用：例如，可以用作平台。在这种情况下，Gradle缺少的信息是，如果使用平台的成员之一，则应将平台添加到依赖项中。[](/md/在项目之间共享依赖版本_md#导入Maven%20BOM)`com.fasterxml.jackson:jackson-bom`
